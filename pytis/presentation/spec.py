...
class ViewSpec(object):
    ...
    def _init(self, title, fields, singular=None, layout=None, list_layout=None, columns=None,
        ...
        check = tuple(computer(c) for c in xtuple(check))
        if __debug__:
            for c in check:
...
"""Value of 'restrict_navigation' preventing navigation altogether.

    Technically it is a URI prefix which no URI can match (it has no scheme), so
    the browser denies any navigation.  Suitable for a document generated by the
    application itself, where all the links are known in advance.

"""

SAME_ORIGIN = 'SAME_ORIGIN'
"""Value of 'restrict_navigation' allowing navigation within the loaded URI origin.

    Only navigation to URIs with the same scheme and authority (host and port)
    as the URI loaded into the browser is allowed.  Only applies when the
    displayed content is an URI -- there is no origin to restrict to otherwise.

"""


class ViewerContent(object):
    """Abstract base class of content specifications for viewer forms.

    Instances of the derived classes define the content displayed by a viewer
    form defined by 'Binding'.  They only describe what to display and under
    which navigation policy -- the display itself is performed by the form (see
    'pytis.form.WebForm').
    ...
    def __init__(self, content):
        ...
        Arguments:
          content: The content to be displayed or a function of one argument (the
            main form 'PresentedRow' instance) returning it.  The kind of the
            content is defined by the derived class.  See also 'ColumnContent'
            for reading the content from a column of the main form.
    ...
    def content(self, row, arguments=None):
        ...
        Arguments:
          row: The main form 'PresentedRow' instance.
          arguments: Table arguments of the form which 'row' belongs to.  Only
            relevant for the derived classes which read the content from the
            database (see 'ColumnContent').
...
class WebContent(ViewerContent):
    """Web content to be displayed in an embedded browser.

    The content is an URI, an HTML string or an 'lcg.Content' instance.  An URI
    is recognized by its scheme, so it must always include one (such as
    'https://').

    """

    def __init__(self, content, base_uri='', resource_provider=None,
                 restrict_navigation=None, on_navigation=None):
        """Initialize the instance.

        Arguments:
          content: URI, HTML string or 'lcg.Content' instance (or a function
            returning it), as described in the parent class.
          base_uri (str): Base URI of the document.  Relative URIs within the
            document are relative to this URI.  Only applies to HTML strings
            ('lcg.Content' instances are exported with their own base URI).
          resource_provider: 'lcg.ResourceProvider' instance providing external
            resources for the document (images, scripts, stylesheets).  Only
            applies to HTML strings ('lcg.Content' instances carry their own
            resources).
          restrict_navigation: Restriction of further navigation within the
            browser.  May be a URI prefix (string), which only allows navigation
            to URIs starting with given prefix, a function of one argument (the
            URI) returning True to allow the navigation and False to prevent it,
            'NO_NAVIGATION' to prevent any navigation at all, 'SAME_ORIGIN'
            (only applies when the content is an URI) to allow navigation within
            its origin, or None to allow any navigation.
          on_navigation: Function called before the browser navigates to a URI.
            ...
            ('pytis.api.Browser' instance).  In a side form, the main form
            'PresentedRow' instance is passed in addition as the keyword
            argument 'row'.  The returned value decides what happens next: True
            (or None) allows the navigation, False prevents it and a function
            prevents it and is called (with no arguments) once the navigation is
            canceled.  The last case makes it possible to perform an arbitrary
            action instead of loading the URI, such as processing the result of
            an authorization performed by an external service.  The action must
            be returned rather than performed directly, because 'on_navigation'
            is called while the browser decides about the navigation, where
            longer operations (network access, dialogs) would block it.

        """
        assert on_navigation is None or callable(on_navigation), on_navigation
        super(WebContent, self).__init__(content)
    ...
    def restrict_navigation(self):
        return self._restrict_navigation

    def on_navigation(self):
        return self._on_navigation


class PdfContent(ViewerContent):
    """PDF document to be displayed in an embedded PDF viewer.

    The content is the document as 'bytes', a file like object or a
...
class ColumnContent(object):
    ...
    Combined with a 'ViewerContent' subclass, it defines a side form displaying
    the value of given main form column.  The constructor takes the column
    identifier instead of the content and the remaining arguments are passed to
    the 'ViewerContent' subclass.

    """

    def __init__(self, column, **kwargs):
        assert isinstance(column, basestring), column
        super(ColumnContent, self).__init__(column, **kwargs)
...
class PdfColumnContent(ColumnContent, PdfContent):
    """PDF document read from a main form column, see 'ColumnContent'."""
...
class Binding(object):
    ...
    def __init__(self,
                 ...
                 content=None,  # type: Optional[Union[str, Callable[..., Any], ViewerContent]]
        ...
        Arguments:
          ...
          content: `ViewerContent` instance defining the content to be displayed
            in the side form.  If not None, the binding is not a standard data
          ...
          content_type (str): Deprecated.  Only applies when `content` is not a
            `ViewerContent` instance, in which case it is converted to one
            according to this value: `'pdf'` to `PdfContent`, anything else to
            `WebContent` (or to their `ColumnContent` variants when `content` is
            a column identifier).  Note that `WebContent` covers URIs, HTML
            strings and `lcg.Content` instances alike -- they are distinguished
            by the value itself, so declaring the type is no longer necessary.
            `'uri'` additionally implies `restrict_navigation=SAME_ORIGIN`,
            which used to be hardwired for this content type.
          uri: Deprecated.  Use content with a `WebContent` instance.
        ...
        if name is not None:
            ...
            if uri is not None:
                ...
                content = uri
                content_type = 'uri'
            if isinstance(content, ViewerContent):
                assert content_type is None, \
                    "'content_type' makes no sense together with a 'ViewerContent' instance."
            else:
                # Convert the deprecated form of the specification, so that the
                # rest of the code only deals with 'ViewerContent' instances.
                assert isinstance(content, basestring) or callable(content), content
                assert content_type in (None, 'html', 'lcg', 'uri', 'pdf'), content_type
                if isinstance(content, basestring):
                    # A string used to be the identifier of the main form column.
                    cls = {'pdf': PdfColumnContent}.get(content_type, WebColumnContent)
                else:
                    cls = {'pdf': PdfContent}.get(content_type, WebContent)
                if content_type == 'uri':
                    # This content type used to imply the same origin restriction.
                    content = cls(content, restrict_navigation=SAME_ORIGIN)
                else:
                    content = cls(content)
