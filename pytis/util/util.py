...
def translation_path():
    ...
    list is based on the environment variable PYTIS_TRANSLATION_PATH (which
    contains the directory names separated by colons).  Pytis and LCG
    translation directories are added automatically.
    ...
    if _translation_path is None:
        _translation_path = []
        env_var = os.getenv('PYTIS_TRANSLATION_PATH')
        if env_var:
            _translation_path.extend(d for d in env_var.split(':'))
        for modname in ('pytis', 'lcg'):
            try:
