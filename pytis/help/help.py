...
class HelpGenerator(object):
    ...
    def help_page(self, uri):
        ...
        """
        root = self._root()
        # Links to sections within the page carry a fragment which is not a part
        # of the page identification.  The browser gets it in 'base_uri', so it
        # can scroll to the target section itself.
        uri = uri.split('#')[0]
