...
def _pypg_new_connection(spec, data):
    ...
    if not isinstance(spec, DBConnection):
        spec = spec()
    if __debug__: log(EVENT, 'Vytvářím nové DB spojení', spec)
...
class DBColumnBinding(DBBinding):
    ...
    def __init__(self, id, table, column, related_to=None, enumerator=None,
        ...
        automaticky vytvořen enumerátor typu 'DataEnumerator', jehož
        konstruktoru budou předány také případné argumenty 'value_column' a
        'validity_column'.
