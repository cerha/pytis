...
class DBData(Data):
    ...
    def __init__(self, bindings, ordering=None):
        ...
        log(DEBUG, 'Klíč databázové instance:', key)
    ...
    def get(self, connection_spec, data=None):
        ...
        try:
            ...
            if connections:
                ...
                log(DEBUG, 'Vytvořeno nové spojení:', c)
        finally:
            lock.release()
        log(DEBUG, 'Předávám spojení:', c)
        return c
...
class DBDataPostgreSQL(DBData):
    ...
    def close(self):
        log(DEBUG, 'Explicitní ukončení selectu')
        ...
        try:
            ...
            if (not self._ordering or \
               ...
               self._pg_already_present(row):
                result = None, False
                log(ACTION, 'Řádek s tímto klíčem již existuje')
            else:
                ...
                if positioned and (not neighbor):
                    log(ACTION, 'Zadaný sousední řádek nenalezen',
                        (after, before))
                    result = None, False
                else:
        ...
        try:
            ...
            if origrow:
                ...
                if new_key != key and self._pg_already_present(row):
                    result = None, False
                    log(ACTION, 'Řádek s tímto klíčem již existuje', key)
                else:
                ...
                result = None, False
                log(ACTION, 'Řádek s daným klíčem neexistuje', key)
        except Exception, e:
            try:
                self._pg_rollback_transaction()
            except:
...
def _pypg_new_connection(spec, data):
    ...
    if not isinstance(spec, DBConnection):
        spec = spec()
    log(DEBUG, 'Vytvářím nové DB spojení', spec)
        ...
        for b in bindings:
            ...
            if enumerator:
                if btype:
                    ...
                    # constraints se momentálně moc nevyužívají.  Pokud to bude
                    # potřeba, je otázka, jak je zde předávat.
...
class DBColumnBinding(DBBinding):
    ...
    def __init__(self, id, table, column, related_to=None, enumerator=None,
        ...
        Argumenty:
          ...
          enumerator -- 'None' nebo sekvence, jejímž prvním prvkem je instance
            třídy 'DataFactory' odpovídající nějakému číselníku a dalšími
            prvky jsou prvky sekvence pro argument 'column_values' třídy
            'pytis.data.Codebook'; může být též přímo instance třídy
            'DataFactory', pak bude jako 'column_values' předán prázdný seznam
          type_ -- explicitně specifikovaný typ sloupce jako instance třídy
            ...
            případ může 'type_' být i dvojice (CODEBOOK_CLASS, DATA_FACTORY),
            kde CODEBOOK_CLASS je třída 'Codebook' nebo její podtřída a
            DATA_FACTORY je instance třídy 'DataFactory'; taková specifikace
            určuje odpovídající číselníkový typ bez nutnosti vytvářet ihned
            v okamžiku jeho specifikace jeho instanci a potažmo datový objekt
            daný DATA_FACTORY.
