...
class Binary(_TypeCheck):
    _test_instance = pytis.data.Binary()
...
class DBCrypto(_DBBaseTest):
    ...
    def tearDown(self):
        # TODO: We should be able to get the drop statements from gsql...
        for q in ("drop table cfoo",
                  "drop function enabletriggers(name)",
                  "drop function disabletriggers(name)",
                  "drop function pytis_crypto_generate_key(integer)",
                  "drop table pytis_crypto_db_keys cascade",
                  "drop table _changes cascade",
                  "drop table _changes_statistic cascade",
                  "drop table _deletes cascade",
                  "drop table _inserts cascade",
                  "drop table _updates cascade",
                  "drop table t_changes cascade",
                  "drop table t_changes_detail cascade",
                  "drop table log cascade",
                  "drop aggregate comma_concat(text)",
                  "drop aggregate space_concat(text)",
                  "drop aggregate line_feed_concat(text)",
                  "drop sequence tempnames_seq",
                  "drop type pytis_crypto_t_key_pair",
                  "drop type pytis_crypto_t_user_contact",):
            try:
                self._sql_command(q)
            except Exception:
                pass
