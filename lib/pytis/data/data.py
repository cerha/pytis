...
class Operator:
    ...
    Třída má pouze specifikační charakter, její instance slouží pro zápis
    podmínek metody.  Každý operátor je celkově dán svým jménem, argumenty
...
class Data(object):
    ...
    def __init__(self, columns, key, ordering=None, **kwargs):
        ...
        Argumenty:
          ...
          columns -- sekvence instancí třídy 'ColumnSpec', tato sekvence
            jednoznačně určuje sloupce tabulky včetně jejich pořadí, které je
            dáno jejich pořadím v 'columns'
          key -- klíčový sloupec nebo seznam klíčových sloupců, jejichž
            hodnoty jednoznačně identifikují řádky tabulky; všechny tyto
            sloupce musí být prvky 'columns'
          ordering -- specifikace automaticky udržovaného třídění, id sloupce
            nebo tuple ids sloupců nebo 'None'.  Není-li 'None', je automaticky
            udržováno pořadí řádků dle daného sloupce (první z uvedených ids),
            který musí být typu `types_.Integer'.  Pokud je argumentem tuple,
            je pořadí udržováno vždy pouze v rámci řádků, které mají stejné
            hodnoty v druhém až posledním z uvedených sloupců.
          kwargs -- předáno předkovi
    ...
    def columns(self):
        """Vrať specifikaci sloupců zadanou v konstruktoru, jako tuple."""
    ...
    def row(self, key):
        """Vrať řádek identifikovaný 'key' jako instanci 'Row'.

        Pokud takový řádek neexistuje, vrať 'None'.

        Argumenty:
        
          key -- instance nebo sekvence instancí třídy 'types_.Value'
            odpovídajících sloupcům klíče reprezentující hodnoty klíče
            požadovaného řádku

        Délka 'key' musí odpovídat počtu klíčových sloupců.
    ...
    def select(self, condition=None, reuse=False, sort=()):
        """Inicializuj natažení všech sloupců z datového zdroje.

        Metoda sama nemusí ještě ještě žádná data natahovat, pouze tento přenos
        inicializuje, je-li třeba.  Metoda vrací počet řádků, které budou
        následně k dispozici; pokud tato hodnota není známa, metoda vrátí
        'None'.  Po zavolání této metody je možno datové řádky získat
        opakovaným voláním metody 'fetchone()'.  Přeskakovat řádky lze metodu
        'skip()'.

        Opakované volání metody nemusí inicializovat zpřístupnění shodných dat
        jako v předchozím volání, pokud se data u datového zdroje mění.

        Argumenty:

          condition -- podmínka omezující výběr řádků; instance třídy
            'Operator' nebo 'None'
          reuse -- právě když je pravda, instance smí pro select použít data
            z předchozího dotazu, shoduje-li se jeho podmínka
          sort -- sekvence specifikátorů třídění, každý specifikátor má podobu
            ID nebo (ID, DIRECTION), kde ID je id tříděného sloupce a DIRECTION
    ...
    def select_map(self, function, **kwargs):
        ...
        Argumenty:
          ...
          function -- funkce jednoho argumentu, jímž je instance třídy 'Row'
          kwargs -- argumenty předané metodě 'select()'
    ...
    def select_aggregate(self, operation, condition=None):
        ...
        Metoda je provádí select, jehož hodnotou je výsledek agregační funkce
    ...
    def lock_row(self, key):
        """Zamči řádek odpovídající 'key' pro editaci a mazání.

        Argumenty:

          key -- klíč řádku, který má být zamčen

        Jestliže 'key' neodpovídá žádnému existujícímu řádku, je chování metody
        nedefinováno.

        Zamykání je neblokující, pokud je již požadovaný řádek zamčen, metoda
        okamžitě skončí, bez toho aniž by se sama pokoušela o jeho další
        zamykání.

        Vrací: 'None', jestliže byl řádek zamčen, slovní popis existujícího
        zámku, jestliže řádek je již zamčen.  Slovní popis obsahuje opravdu jen
        popis zámku, ne tedy například informaci o tom že záznam je zamčen,
        začíná malým písmenem a není ukončen žádným interpunkčním znaménkem.

        Cílem zamykání je informovat ostatní klienty, že příslušný záznam je
        editován a že by s ním neměli manipulovat.  Zamykání nemusí technicky
        zabraňovat modifikacím u zdroje dat, ke kterému se přistupuje jinak než
        instancí této třídy.  Není také nutno zajišťovat ošetření přístupu
        k datům z více threadů, jestliže třída samotná není thread safe.

        V každý okamžik může být zamčen nejvýše jeden řádek jedné instance této
        třídy.  Při pokusu o zamčení dalšího řádku je chování metody
        nedefinováno.
    ...
    def unlock_row(self):
        """Odemči aktuálně zamčený řádek.

        Jestliže není zamčen žádný řádek, nedělej nic.

        """
        self._locked_row = None

    def locked_row(self):
        """Vrať klíč aktuálně zamčeného řádku.

        Jestliže momentálně není zamčený žádný řádek, vrať 'None'.
...
class MemData(Data):
    ...
    def select(self, condition=None, reuse=False, sort=None):
        ...
        Bližší popis viz nadtřída.  Argumenty 'condition' a 'sort' jsou
        ignorovány.
...
def AND(*args):
    ...
    Argumenty:
      ...
      args -- sekvence operátorů (může být i prázdná)

    Operátor je komutativní, na pořadí argumentů tedy nezáleží.
...
class DataFactory(object):
    ...
    def create(self, **kwargs):
        ...
        # TODO: Cachovaní datových objektů zatím nemůžeme použít,
        #       protože narážíme na problém udržovaných konexí
