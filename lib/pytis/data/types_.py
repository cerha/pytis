...
class Type(object):
    ...
    def enumerator(self):
        """Vrať enumerátor svázaný s tímto typem."""
...
class Limited(Type):
    ...
    _VM_MAXLEN_EXCEEDED_MSG = _("Překročena maximální délka %(maxlen)s")
...
class String(Limited):
    ...
    _VM_MAXLEN_EXCEEDED_MSG = _("Řetězec přesahuje maximální délku %(maxlen)s")
    _SPECIAL_VALUES = Type._SPECIAL_VALUES + ((None, ''),)
...
class Binary(Limited):
    ...
    _VM_MAXLEN_EXCEEDED_MSG = _("Překročena maximální velikost %(maxlen)sB")
...
class FixedEnumerator(Enumerator):
    """Enumerátor pracující s fixní množinou hodnot."""
    ...
    def __init__(self, enumeration):
        ...
        Argumenty:
          ...
          enumeration -- sekvence hodnot kompatibilních s vnitřními
            (Pythonovými) hodnotami typu, pro který má být enumerátor použit.
...
class MutableEnumerator(Enumerator):
    """Abstraktní třída, kterou povinně dědí všechny mutable enumerátory.

    Mutable enumerátor takový, jehož množina hodnot se může v čase měnit.
    ...
    def _update(self, force=False):
        ...
        Bezprostředně po zavolání této metody by měl enumerátor pracovat
        s aktuálními daty.
        ...
        Vrací: Pravdu, právě když byl update skutečně proveden.
...
class DataEnumerator(MutableEnumerator):
    ...
    Hodnoty výčtu jsou pythonové hodnoty určitého sloupce datového objektu.
    Typicky je to klíčový sloupec ale pomocí argumentů konstruktoru je možno
    ...
    def __init__(self, data_factory, data_factory_kwargs={}, value_column=None,
        ...
        Argumenty:
          ...
          data_factory -- instance třídy 'DataFactory' sloužící k vytvoření
            datového objektu použitého k získání výčtových hodnot.
          ...
          value_column -- id sloupce datového objektu poskytujícího hodnoty
            enumerátoru.  Je-li None, bude použit klíčový sloupec.
            
          validity_column -- id sloupce, určujícího platnost řádků datového
            zdroje.  Pokud je určen (není None), budou za hodnoty výčtu
            považovány pouze ty řádky, v nichž daný sloupec nabývá pravdivé
            hodnoty (musí jít o Boolean sloupec).
            
          validity_column -- id sloupce, určujícího platnost řádků datového
            zdroje.  Pokud je určen (není None), budou za hodnoty výčtu
            považovány pouze ty řádky, v nichž daný sloupec nabývá pravdivé
            hodnoty (musí jít o Boolean sloupec).  Není možno použít v
            kombinaci s validity_condition.
          
          validity_condition -- podmínka určující platnost řádků datového
            zdroje.  Pokud je určena, budou za hodnoty výčtu považovány pouze
            ty řádky, v nichž je podmínka pravdivá.  Jde o obecnější variantu
            validity_column a nelze použít v kombinaci s tímto argumentem.
    ...
    def _complete(self):
        # Dokonči instanci vytvořením datového objektu.
    ...
    def data_factory(self):
        """Vrať specifikaci datového jako instanci 'pytis.data.DataFactory'."""
    ...
    def get(self, value, column=None):
        """Získej z dat hodnotu daného sloupce z řádku odpovídajícího 'value'.

        Argumenty:
        
           value -- vnitřní (Pythonová) hodnota sloupce 'value_column' z
             datového objektu.  Podle této hodnoty bude vyhledán příslušný
             řádek.
           column -- identifikátor sloupce datového objektu, jehož hodnota má
             být vrácena.

        Vrací instanci 'Value', nebo None, pokud daný řádek nebyl nalezen.
    ...
    def iter(self):
        """Vrať iterátor, iterující přes všechny datové řádky."""
        # TODO: Asi by bylo čistší předefinovat metodu values a tu potom
        # používat v kombinaci s metodou get.  Aby se ovšem v get neprováděl
        # zbytečně nový select, bylo by nutné si nějak interně pamatovat
        # poslední řádek a v metodě get jej potom rovnou použít, pokud je to
        # řádek požadované hodnoty.
    ...
    def set_runtime_filter_provider(self, provider, args):
        """Nastav poskytovatele run-time podmínky filtrující řádky enumerátoru.
        ...
        Argumenty:
          ...
          provider -- None, nebo funkce, která vrací instanci třídy 'Operator'.
            ...
            filtrovací podmínku.
          args -- seznam argumentů (tuple), které mají být předány této funkci.

        Run-time podmínka umožňuje měnit množinu platných řádků enumerátoru za
        běhu.  Po externí změně podmínky je třeba toto oznámit voláním
        'notify_runtime_filter_change()'.  Tím je zajištěno, že při všech
        následných operacích s enumerátorem bude podmínka automaticky
        přepočítána a množina platných hodnot enumerátoru bude aktualizována.
    ...
    def notify_runtime_filter_change(self):
        ...
        run-time filtrovací podmínky.  Enumerátor se tak dozví, že si má v
    ...
    def validity_condition(self):
        """Vrať podmínku určující platné řádky enumerátoru.
        
        Podmínka zahrnuje jak statické omezení řádků enumerátoru, tak aktuální
