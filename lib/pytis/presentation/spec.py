...
class ViewSpec(object):
    ...
    def __init__(self, title, fields, singular=None, layout=None, columns=None,
        ...
        Argumenty:
          ...
          redirect -- přesměrování formulře pro editaci jednoho záznamu.  Jedná
            se o funkci jednoho argumentu, jímž je instance 'PresentedRow'
            reprezentující řádek dat, pro který je přesměrování požadováno.
            Vrácenou hodnotou musí být název specifikace, nad kterou bude
            vytvářený formulář sestaven.  Pokud funkce vrátí None, nebo není
            žádná funkce specifikována, k žádnému přesměrování nedojde.
            Stejného efektu by šlo dosáhnout také pomocí 'on_edit_record', ale
            použití 'redirect' je v podstatě zjednodušená varianta
            'on_edit_record'.  Pokud je však 'on_edit_record' použit, je
            'redirect' ignorován!
    ...
    def conditions(self):
        """Vrať tuple předdefinovaných filtračních/vyhledávacích podmínek."""
...
class BindingSpec(object):
    ...
    def __init__(self, title, binding_column, side_binding_column=None,
        ...
        Argumenty:
          ...
          title -- titulek daného duálního spojení formulářů jako řetězec.
...
class FieldSpec(object):
    ...
    def __init__(self, id, label='', column_label=None, descr=None,
        ...
        Argumenty:
          ...
          disable_column -- Pokud je udána pravdivá hodnota, políčko nebude možné
            zobrazit jako sloupec -- tj. nebude se vyskytovat ve výběru
            sloupců, které je možno zobrazit.  Pokus o umístění takového
            políčka do výchozích sloupců (argument 'columns' ve 'ViewSpec')
            bude považován za chybu a ohlášen.
          ...
          default -- funkce pro výpočet výchozí hodnoty políčka.  Může být
            buďto přímo hodnota kompatibilní s vnitřní hodnotou odpovídajícího
            datového typu, nebo callable objekt (bez argumentů) tuto hodnotu
            vracející.  Pokud je 'None' bude použita výchozí hodnota určená
            datovým typem.
          ...
          thumbnail -- identifikátor políčka poskytujícího zmenšený náhled
            tohoto políčka.  Relevantní jen pro políčka typu
            'pytis.data.Image'.
    ...
    def disable_column(self):
        """Vrať pravdu, pokud políčko není zobrazitelné jako slopec."""
    ...
    def default(self):
        """Vrať výchozí hodnotu zadanou v konstruktoru."""
        return self._default
    ...
    def thumbnail(self):
        """Vrať identifikátor políčka obsahujícího zmenšeninu obrázku."""
...
class Specification(object):
    ...
    hodnotou argumentu 'description' pro 'ViewSpec' je dokumentační řetězec
    specifikační třídy.
    ...
    oid = ('oid',)
    """Specifikace názvů OID sloupců (tuple řetězců).

    Výchozím nastavením je jeden sloupec s názvem 'oid'.  Pokud tabulka nemá
    mít žádný oid sloupec, uvedeme prázdný seznam.  Pokud je sloupec jen
    jeden, není nutno jej obalovat do tuplu.  Pro všechny uvedené sloupce budou
    automaticky přidány příslušné vazby.
