...
class FieldSpec(object):
    ...
    def __init__(self, id, label='', column_label=None, descr=None,
        ...
        Argumenty:
          ...
          display -- určuje způsob získání zobrazené uživatelské hodnoty.
            Relevantní jen pro políčka výčtových typů (datový typ má definován
            enumerátor).  Pokud je None, bude použita hodnota z 'cb_spec' ve
            specifikaci číselníku (což by mělo být také upřednostňováno).
            Pokud je dispej určen explicitně, může to být buďto identifikátor
            sloupečku v datovém objektu enumerátoru (bude zobrazena hodnota
            tohoto sloupečku), nebo funkce jednoho argumentu (vnitřní Pythonová
            hodnota enumerátoru), která vrací uživatelskou hodnotu (řetězec).
            Může být přdána také dvojice (funkce, identifikátor sloupečku).  V
            tom případě bude argumentem funkce hodnota daného sloupce, namísto
            sloupce vnitřní hodnoty.
