...
class SelectionType:
    """Výčtová třída definující konstanty pro způsob výběru z množiny hodnot.
    """
    CHOICE = 'CHOICE'
    """Výběr, kdy je viditelná jen právě vybraná hodnota."""
    RADIO_BOX = 'RADIO_BOX'
    """Pro každou hodnotu vytvoř zaškrtávací políčko."""
    LIST_BOX = 'LIST_BOX'
...
class CodeBookSpec:
    ...
    def __init__(self, name, returned_column, columns=None, display=None,
        ...
        Argumenty:
          ...
          name -- název specifikace pro resolver (řetězec)
          returned_column -- identifikátor sloupce ze specifikace, jehož
            hodnota má být číselníkem vrácena a doplněna jako hodnota políčka.
          columns -- sekvence identifikátorů sloupců, které mají být zobrazeny
            v číselníkovém formuláři (třída 'CodeBook').  Pokud je 'None',
          ...
          insert_unknown_values -- pokud má pravdivou hodnotu, umožni vkládání
            neznámých hodnot do číselníku přímo při jejich vyplnění do
            číselníkového políčka.  V takovém případě je uživateli nabídnuta
            možnost vložit nový záznam do číselníku, pokud vyplní (jinak
            validní) hodnotu, která není v číselníku přítomna.
    ...
    def name(self):
        """Vrať název specifikace číselníku."""
        return self._name
        
    def returned_column(self):
        """Vrať seznam identifikátorů sloupců, tvořících návratovou hodnotu."""
        return self._returned_column
        
    def returned_columns(self):
        """Pouze pro zpětnou kompatibilitu.  Nepoužívat!."""
        return (self._returned_column,)
    ...
    def insert_unknown_values(self):
        """Vrať pravdu, pokud mají být do číselníku vkládány neznámé hodnoty."""
...
class RefSpec:
    """Specifikace seznamu závislých záznamů pro vstupní políčko 'ListField'.

    Tato specifikace je určena pro použití jako argument 'references'
    specifikace vstupního pole (třída 'FieldSpec').
    ...
    def __init__(self, name, key, columns, sorting=(), returned_columns=None):
        ...
        Argumenty:
          ...
          name -- název specifikace závislé tabulky pro resolver.  Data pro
            zobrazený seznam, jakožto i prezentační vlastnosti sloupců jsou
            určena touto specifikací získanou pomocí resolveru.
          key -- id sloupce v závislé tabulce, jehož hodnota se musí shodovat s
            vnitřní hodnotou políčka (určenou přes 'computer'), aby byl
            odpovídající záznam v sezamu zobrazen.
          columns -- tuple identifikátorů sloupců ze závislé tabulky, které
            mají být v seznamu viditelné.
          sorting -- určuje způsob řazení záznamů ve formátu argumentu 'sort'
            metody 'pytis.data.Data.select()'.
          returned_columns -- seznam dvojic ('menutext', 'id_sloupce'), kde
            menutext se zobrazí v popup_menu a id_sloupce se použije pro
            nastavení hodnoty get_value(). Pokud je None, pak se vrací
            implicitní hodnota (tedy self._value) a popup_menu se nevytváří.  
    ...
    def name(self):
        """Vrať jméno specifikace závislé tabulky pro resolver."""
        return self._name

    def key(self):
        """Vrať id sloupce, přes který jsou vybírány závislé záznamy."""
        return self._key

    def columns(self):
        """Vrať seznam id sloupců, které mají být zobrazeny."""
        return self._columns

    def sorting(self):
        """Vrať specifikaci třídění."""
        return self._sorting

    def returned_columns(self):
        """Vrať seznam specifikací pro vracené sloupce."""
...
class FieldSpec:
    ...
    def __init__(self, id, label='', column_label=None, descr=None,
        ...
        Argumenty:
          ...
          separator -- oddělovač jednotlivých hodnot vícehodnotového
            sloupečku. string.  Relevantní pouze pokud má navázaný datový
            sloupeček vícenásobnou vazbu na zdroj dat, data jsou pak spojena
            tímto oddělovačem do jediného stringu.
          line_separator -- oddělovač řádků v jednořádkovém zobrazení
            víceřádkové hodnoty.  Tento argument smí být využíván pouze pro
            read-only políčka.
          codebook -- specifikace číselníku, pokud je políčko na nějaký vázáno;
          ...
          references -- specifikace vazby na jinou tabulku, pro políčko
            zobrazující seznam závislých záznamů.  Pokud není None, jedná se o
            instanci třídy 'RefSpec'.  V takovém případě musí být nutně určen
            také 'computer', protože seznamové políčko je svou podstatou
            virtuální -- jedná se o výpis závislých záznamů z jiné tabulky -- a
            jeho vnitřní hodnota, určená pomocí computeru, slouží jako klíč do
            této závislé tabulky.  Toto políčko není samostatně nijak
            editovatelné, pouze zobrazuje data.  Umožňuje však vyvolat
            samostatný formulář pro zobrazená data.  Blíže viz dokumentace
            'pytis.form.ListField'.
          selection_type -- způsob výběru z množiny hodnot, jedna z konstant
            třídy 'SelectionType'; relevantní jen pro vstupní pole dat
            výčtových typů.
          ...
          filter_list -- seznam povolených, nebo zakázaných znaků, pokud je
            hodnotou atributu 'filter' konstanta 'INCLUDE_LIST' nebo
            'EXCLUDE_LIST'.  Jedná se o sekvenci znaků.
    ...
    def separator(self):
        """Vrať oddělovač jednotlivých hodnot vícehodnotového sloupečku."""
    ...
    def references(self):
        """Vrať specifikaci seznamu závislých záznamů."""
...
class Spec(object):
    ...
    Specifikační funkce často používají klíčové argumenty k rozlišení
    jednotlivých variant specifikací vycházejících ze stejného základu.
    Kombinace názvu specifikace a argumentů specifikační funkce určují každou
    takovou variantu.  Tato třída zabaluje tyto parametry to jediné instance
    ...
    def __init__(self, name, **kwargs):
        ...
        Argumenty:
          ...
          name -- jméno specifikačního souboru jako řetězec.
          **kwargs -- argumenty, které budou předány specifikační funkci při
    ...
    def name(self):
        """Vrať název specifikačního modulu jako řetězec."""
        return self._name

    def kwargs(self):
        """Vrať klíčové argumenty specifikační funkce jako slovník."""
        return self._kwargs
