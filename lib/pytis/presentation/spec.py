...
class CodebookSpec(object):
    ...
    def __init__(self, columns=None, sorting=None, display=None,
        ...
        Argumenty:
          ...
          display -- pokud není 'None', budou číselníková políčka vázaná na
            tento číselník automaticky vybavena displejem, (viz
            'CodebookField').  Hodnotou je identifikátor sloupce obsahujícího
            hodnotu k zobrazení v displeji (tento sloupec musí být obsažen v
            datové specifikaci číselníku).
...
class Link(object):
    ...
    def __init__(self, name, column, form=None, label=None):
        ...
        Argumenty:
          ...
          form -- typ formuláře, ve kterém bude odkazovaný náhled otevřen
            (instance třídy 'Form').  Výchozím typem je buďto 'BrowseForm',
            nebo 'DualBrowseForm' pro duální náhledy (name je složený název).
...
class FieldSpec(object):
    ...
    def __init__(self, id, label='', column_label=None, descr=None,
        ...
        Argumenty:
          ...
          display -- identifikátor sloupečku číselníku, který má být použit pro
            zobrazení uživatelské hodnoty.  Relevantní jen pro číselníková
            políčka.  Pokud je None, bude použita hodnota z 'cb_spec' ve
            specifikaci číselníku (což by mělo být také upřednostňováno).  Může
            být také funkce jednoho argumentu (vnitřní Pythonová hodnota
            číselníku), která vrací uživatelskou hodnotu (řetězec).
          ...
          link -- specifikace odkazu do jiného náhledu souvisejícího s hodnotou
            políčka.  Instance 'Link'.  V kontextovém menu řádku bude pro každý
            odkaz vytvořena jedna položka umožňující odskok do odkazovaného
            náhledu s vyhledáním záznamu odpovídajícího aktuální hodnotě
            políčka.
        ...
        explicitně datový typ pomocí specifikátoru 'type_', protože není možné
