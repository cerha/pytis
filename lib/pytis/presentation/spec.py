...
class ViewSpec(object):
	...
	soubor ve zdrojovém adresáři nápovědy (viz tutoriál Help).
        ...
        if popup_menu is not None:
            #log(OPERATIONAL,
            #    "Použit potlačený argument 'popup_menu' třídy 'ViewSpec'!")
...
class FieldSpec(object):
    ...
    def __init__(self, id, label='', column_label=None, descr=None,
        ...
        Argumenty:
          ...
          codebook -- název specifikace číselníku (řetězec), nebo None.
            Implicitně je název specifikace číselníku přebírán ze specifikace
            enumerátoru datového typu odpovídajícího sloupce v 'DataSpec'.
            Pokud však z nějakých důvodů chceme v uživatelském rozhraní
            pracovat s jiným číselníkem (např. jeho jinou variantou) je možné
            jej zde předefinovat.  Tento název je zároveň využíván k vytvoření
            odskoku na číselník v kontextovém menu buňky.  Pro sloupce s
            enumerátorem je číselník určen tímto enumerátorem, ale pokud
            sloupec enumerátor nemá a přesto jde o hodnotu pocházející z
            číselníku (např. je do tabulky zahrnuta v rámci view na úrovni
            databáze), je explicitní určení názvu číselníku pomocí tohoto
            specifikátoru jedinou možností.
