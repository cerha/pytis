...
class PresentedRow(object):
        ...
        if row is None:
            def genval(c):
                ...
                if prefill is not None and prefill.has_key(key):
                    value = prefill[key]
                    # Pro Codebooky raději taky vytvoříme novou instanci
                    ...
                    if self._columns.has_key(key):
                        ...
                        if self._new and default is not None:
                            ...
                            # Je to tu jen proto, že v defsech jsou nyní všude
                            # požívány computery s depends=() namísto
                            # 'default', protože default dříve nefungovalo
                            # korektně.  Až se to ve všech defsech opraví zpět
                            # na použití 'default', je NUTNÉ následující
                            # podmínku zrušit!
    ...
    def _resolve_dependencies(self, key=None):
        ...
        if invoke_callbacks and self._change_callback is not None:
            ...
            # editovatelnosti a runtime codebooků, nebo zůstala "dirty" a musíme
            # tedy jejich callback zavolat teď.
    ...
    def refvalue(self, key):
        """Metoda existuje jen kvůli zpětné kompatibilitě, ale vrací vždy None.

        Brzy bude zrušena, takže je třeba všechna její použití odstranit.
        
        """
        log(EVENT, "Použita potlačená metoda 'PresentedRow.refvalue()'!")
        # TODO: Zrušit!!!
        return None
