...
class SFDialog(SFSDialog):
    ...
    def _create_controls(self):
        ...
        try:
            ...
            run_dialog(Warning, _("Nepodařilo se rozložit podmínkový výraz:") +" "+ str(e))
    ...
    def _create_content(self, sizer):
        ...
        buttons = [
            button(_("Přidat AND"), lambda e: self._on_add(),
                   tooltip=_("Přidat novou podmínku v konjunkci (a zároveň)")),
            button(_("Přidat OR"), lambda e: self._on_add(or_=True),
                   tooltip=_("Přidat novou podmínku v disjunkci (a nebo)")),
            button(_("Odebrat vše"), lambda e: self._on_reset(),
                   tooltip=_("Zrušit všechny stávající podmínky"))]
            ...
            if col2 is not self._NO_COLUMN:
                ...
                for basetype in (pytis.data.String, pytis.data.Number, pytis.data.DateTime,
                    ...
                    if isinstance(col1.type(), basetype) and not isinstance(col2.type(), basetype):
                        raise self.SFConditionError(i, wcol2, _("Neslučitelné typy %s a %s") %
                ...
                if wval.GetValue():
                    ...
                    raise self.SFConditionError(i, wop,
                                     _("Binární sloupec lze testovat pouze na rovnost či nerovnost"))
