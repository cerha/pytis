...
class SFDialog(SFSDialog):
        ...
        def condition(id):
            ...
            buttons = [self._create_button(label, handler, tip=tooltip)
                       ...
                       ((_("Smazat"), _("Vymazat obsah podmínky"),
                        ...
                        (_("Nasát"), _("Načíst hodnotu aktivní buňky"),
                         lambda e: self._on_suck(e, id)),
                        (_("Odebrat"), _("Zrušit tuto podmínku"),
    ...
    def append_condition(self, col_id, value):
        ...
        'col_id' na hodnotu 'value'.  Podmínka je přidána vždy v konjunkci se
    ...
    def reset_condition(self):
        """Neinteraktivně zruš stávající filtrovací podmínku.

        Pozor, podmínka se přestane uplatňovat pouze pro všechny následující
        operace bez otevření dialogu (append_condition).  Při následném
        otevření dialogu jsou však předchozí podmínky stále přítomné a při
        novém vyfiltrování se uplatní.  Zrušení podmínek z dialogu je nutno
        provést ručně.
    ...
    def run(self, current_row, current_field):
        ...
        Argumenty:
          ...
          current_row -- aktuální řádek formuláře jako instance třídy
            ...
            'None'
          current_field -- identifikátor aktuálního políčka/sloupce.  Je-li
            'None', bude vybrán nějaký implicitní sloupec.
...
class FilterDialog(SFDialog):
    ...
    def run(self, data, filter, current_row, current_field):
        ...
        Argumenty:
          ...
          data -- datový objekt nad nímž budou prováděny případné agregační
            funkce.
          filter -- případná doplňující podmínka filtrující data datového
            objektu při výpočtu agregačních funkcí.  Tato podmínka bude
            uplatněja současně s aktuální podmínkou nastavenou v dialogu.
