...
class SortingDialog(SFSDialog):
    ...
    _TITLE = _("Třídění")
    _SORT_BUTTON = _("Setřídit")
    _RESET_BUTTON = _("Resetovat třídění")
        ...
        for cid, dir in self._sorting or ((self._col, self._direction),):
            ...
            self._controls.append((
                choice(self._columns, selected=self._find_column(cid),
                       ...
                       tooltip=_("Zvolte sloupec, podle nějž chcete třídit")),
...
class SFDialog(SFSDialog):
    ...
    def _create_content(self):
        ...
        buttons = [
            ...
            button(_("Zrušit vše"), lambda e: self._on_reset(),
                   tooltip=_("Přidat novou podmínku v disjunkci"))]
        if self._conditions:
            buttons.append(
    ...
    def _on_remove_saved(self):
        ...
        if not cond.fixed():
            msg = _("Opravdu chcete smazat uloženou podmínku '%s'?") \
                  % cond.name()
