...
class Form(Window, KeyHandler, CallbackHandler):
    ...
    def on_ui_event(self, event, id):
        ...
        Metoda musí událost buď sama ošetřit, nebo ji vypropagovat do vnitřního
        prvku formuláře, pokud takový je a má metodu stejného názvu jako tato.

        Vrací: Pravdu, právě když metoda nebo jí volaná metoda událost
        zpracovala.
...
class RecordForm(Form):
    ...
    """UI test na právo vložení nového záznamu."""
    ...
    def _on_row_change(self, field_id):
        # Signalizace změny políčka z _row
        pass

    def _on_field_enable(self, field_id):
        # Callback pro povolení editovatelnosti políčka
        pass
        
    def _on_field_disable(self, field_id):
        # Callback pro zakázání editovatelnosti políčka
        pass
...
class EditForm(LookupForm, TitledForm):
    ...
    def _validate(self):
        ...
        for f in self._fields:
            ...
            # TODO: Tato kontrola by měla být prováděna spíše v
    ...
    def _field_change(self, id, value):
        # Vezmi na vědomí, že hodnota políčka 'id' byla změněna na 'value'.
        self._row[id] = value    
    ...
    def select_row(self, key):
        """Zobraz záznam s klíčem 'key'.

        Pokud takový záznam neexistuje, zobraz chybový dialog a jinak nic.

        Argumenty:

          key -- klíč požadovaného řádku jako tuple nebo instance třídy
            'pytis.data.Row' kompatibilní s datovým objektem formuláře
...
class ShowForm(EditForm):
    ...
    # TODO: Toto z neznámých důvodů změní velikost rodičovského okna.
    #def _finish_top_level_sizer(self, sizer):
