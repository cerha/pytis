...
class LookupForm(InnerForm):
    ...
    def _cmd_jump(self):
        ...
        if max_value > 0:
            prompt = _("Záznam číslo (1-%s):") % (max_value,)
    ...
    def _cmd_save_profile(self, name, kbd=False):
        ...
        if current_profile != self._default_profile():
            if name == self._UNNAMED_PROFILE_LABEL or not name:
                message(_("Nejprve upravte název, pod kterým chcete profil uložit."), beep_=True)
                return
            for profile in self._view.profiles() + self._user_profiles:
                if profile.name() == name:
                    message(_("Pojmenovaný profil '%s' již existuje.") % name, beep_=True)
                    return
                elif profile == current_profile:
                    message(_("Shodný profil již existuje pod názvem '%s'.") % profile.name(),
                            beep_=True)
                    return
    ...
    def profile_context_menu(self, ctrl):
        ...
        return (
            MItem(_("Uložit"), self.COMMAND_SAVE_PROFILE(name=ctrl.GetValue()),
                  help=_("Uložit současný profil pod tímto názvem"), hotkey='Enter'),
            ...
            and Menu(_("Uložit jako"),
                     [MItem(f.name(),
                            ...
                            help=_("Aktualizovat existující uložený profil."))
...
class PopupEditForm(PopupForm, EditForm):
    ...
    def _exit_check(self):
        ...
        if data is not None and i <= len(data):
            msg = _("Ještě nebyly zpracovány všechny řádky vstupních dat.\n"
    ...
    def _buttons(self):
        buttons = (dict(id=wx.ID_OK,
                        tooltip=_("Uložit záznam a uzavřít formulář"),
                        command=self.COMMAND_COMMIT_RECORD()),
                   dict(id=wx.ID_CANCEL,
                        tooltip=_("Uzavřít formulář bez uložení dat"),
                        command=self.COMMAND_LEAVE_FORM()))
        if self._mode == self.MODE_INSERT and self._multi_insert:
            buttons += (dict(id=wx.ID_FORWARD, label=_("Další"), #icon=wx.ART_GO_FORWARD, 
                             tooltip=_("Uložit záznam a reinicializovat formulář"
                                       " pro vložení dalšího záznamu"),
                             command=self.COMMAND_COMMIT_RECORD(next=True)),)
        if self._inserted_data is not None:
            buttons += (dict(label=_("Přeskočit"),
                             tooltip=_("Přeskočit tento záznam bez uložení"),
