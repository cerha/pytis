...
Command(ListForm, 'CURRENT_ROW_ACTION',
        "Akce nad aktuálním řádkem tabulky")
# 	Tento příkaz je určen k vytváření uživatelem definovaných akcí
#       pracujících s aktuálním řádkem tabulky.  Typické použití je v 
# 	kontextovém menu BrowseFormu.
#
# 	Příkaz může mít libovolné argumenty, avšak tři z nich jsou zpracovány
# 	odlišně:
#
#            `handler' -- je povinný a musí odkazovat na funkci, která akci
#               provede.
#
#            `enabled' -- funkce, vracející pravdu, pokud je příkaz aktivní a
#               nepravdu v opačném případě.  Argument je nepovinný a pokud není
#               uveden, je příkaz aktivní v závislosti na `access_groups'.
#
#            `access_groups' -- seznam skupin, které mají právo příkaz
#               vyvolat.  Příkaz se pro ostatní uživatele stane automaticky
#               neaktivní (teprve pokud uživatel patří do jedné z vyjmenovaných
#               skupin, je zpracováván argument `enabled'.
#
#       Jako první poziční argument bude funkci `handler' i `enabled' předána
#       instance PresentedRow aktuálního řádku tabulky.
#
# 	Dále budou předány všechny klíčové argumenty příkazu, vyjma argumentů
# 	`handler', `enabled' a `access_groups' (ty jsou zpracovávány interně).
#
Command(ListForm, 'SELECTED_ROWS_ACTION',
        "Akce nad všemi právě vybranými řádky tabulky")
#       Obdoba CURRENT_ROW_ACTION, ale zde je handleru předáván iterátor, který
#       vrací všechny právě vybrané řádky tabulky (jako instance PresentedRow).
...
FORM_COMMAND_MENU = ((
    ...
    (_("Hledat další"),         LookupForm.COMMAND_SEARCH(direction=
                                                          pytis.data.FORWARD)),
    (_("Hledat předchozí"),     LookupForm.COMMAND_SEARCH(direction=
                                                          pytis.data.BACKWARD)),
    ...
    (_("Vložit řádku pod"),     ListForm.COMMAND_NEW_LINE()),
    (_("Vložit řádku nad"),     ListForm.COMMAND_NEW_LINE(before=True)),
    (_("Kopírovat řádku pod"),  ListForm.COMMAND_NEW_LINE(copy=True)),
    (_("Kopírovat řádku nad"),  ListForm.COMMAND_NEW_LINE(copy=True,
                                                          before=True)),
    ...
    (_("Náhled v duálním formuláři"), ListForm.COMMAND_ACTIVATE(alternate=True)),
    ))
