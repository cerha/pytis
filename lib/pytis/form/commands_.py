...
Command(ListForm, 'RESIZE_COLUMN',
        "Rozšíření/zůžení sloupce.  Argument 'diff' udává počet +/- pixelů.")
Command(ListForm, 'MOVE_COLUMN',
        "Přesunutí sloupce doprava/doleva.  'diff' udává +/- o kolik sloupců.")
...
Command(ListForm, 'CONTEXT_MENU_ACTION',
        """Akce kontextového menu řádku tabulky.
        
	Je určen k vytváření uživatelem definovaných akcí v kontextovém menu
	řádku BrowseFormu.

	Příkaz může mít libovolné argumenty, avšak dva z nich jsou
	zpracovány odlišně:
           ...
           `enabled' -- funkce, vracející pravdu, pokud je příkaz aktivní a
              ...
              uveden, je příkaz vždy aktivní.

	Funkce `handler' i `enabled' dostanou jako klíčové argumenty všechny
	ostatní argumenty příkazu, vyjma sebe sama (argumentů `handler' a
	`enabled').

        Navíc, pokud tyto funkce přijímají jeden poziční argument,
	dostanou jako první argument instanci PresentedRow aktuálního řádku,
	nad kterým byla akce vyvolána.  Pokud přijímají dva poziční argumenty,
	bude na prvním místě předán ještě datový objekt formuláře.""")
