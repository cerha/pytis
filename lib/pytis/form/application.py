...
class Application(wx.App, KeyHandler):
    ...
    """UI test na přítomnost otevřeného okna v aplikaci."""
    UI_OPENED_MORE_WINDOWS = 'UI_OPENED_MORE_WINDOWS'
    """UI test na přítomnost více než jednoho otevřeného okna v aplikaci."""
    ...
    def run_dialog(self, dialog_or_class_, *args, **kwargs):
        ...
        if top is not None:
            ...
            self._panel.SetFocus()
        top = self
        log(ACTION, "Dialog ukončen:", (dialog, result))
        return result
    ...
    def on_ui_event(self, event, id):
        ...
        Metoda událost propaguje do aktuálního formuláře, je-li jaký.
...
def get_status(id):
    """Vrať text pole 'id' stavové řádky.
...
def current_form():
    ...
    Pokud není otevřena žádná aplikace, nebo současná aplikace nemá jako
...
def exit(**kwargs):
    ...
    Argumenty:
      ...
      kwargs -- předáno metodě 'Application.exit()'
