...
class NullApplication:
    # Toto je hack, který umožňuje natáhnout defsy bez toho, aby běželo
    # uživatelské rozhraní.  Defsy totiž často používají globální funkce z
    # application.  Bude třeba to změnit, abychom se tohoto mohli zbavit.
    # Protože je to jen dočasný hack, nebyla ani snaha deklarovat společné
    # vlastnosti obou aplikací v nadtřídě...
...
class Application(wx.App, KeyHandler, CommandHandler):
    ...
    def run_form(self, form_class, name, *args, **kwargs):
        ...
        odstranit příkazem 'Form.COMMAND_LEAVE_FORM'.
    ...
    def run_procedure(self, spec_name, proc_name, *args, **kwargs):
        ...
        result = None
        try:
            log(ACTION, 'Spouštím proceduru:', (spec_name, proc_name, args, kwargs))
