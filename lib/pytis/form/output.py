...
class PostscriptViewer(wx.ScrolledWindow):
    ...
    def __init__(self, parent, stream, zoom):
        ...
        """
        log(DEBUG, 'Startuji PostScriptovou prohlížečku:', zoom)
    ...
    def _wait_for_gs(self):
        log(DEBUG, 'Čekání na dokončení běhu Ghostscriptu')
        while not self._gs.finished():
            time.sleep(1)
        log(DEBUG, 'Běh Ghostscriptu je dokončen')
    ...
    def show_page(self, page_number):
        ...
        while True:
            ...
            if final or npages >= 1:
                log(DEBUG, 'Nějaké stránky už jsou na skladě')
                break
            log(DEBUG, 'mikrospánek')
            microsleep()
        if npages < 1:
            log(DEBUG, 'Výstup nemá žádnou stránku')
            return 0
        gs = self._gs
        if page_number < 1:
...
class PrintForm(Form):
    ...
    """Aktivační konstanta tiskového formuláře."""
    ...
    def _show_page(self, page_number):
        log(DEBUG, 'Požadavek na zobrazení stránky:', page_number)
        preview = self._preview
    ...
    def _zoom(self, zoom):
        log(DEBUG, 'Požadavek na zoom:', zoom)
        if zoom < 10:
            zoom = 10
