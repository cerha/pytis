...
def top_level_exception():
    ...
    log(OPERATIONAL, 'Odchycena výjimka na nejvyšší úrovni', tbstring)
    ...
    if text is None:
        ...
        if not to:
            run_dialog(Message, _("Není známa cílová adresa. "
                                  "Je nutno nastavit konfigurační volbu "
                                  "`bug_report_address'."))
        else:
            import commands
            ...
            while True:
                address = run_dialog(InputDialog,
                                     prompt=_("Vaše e-mailová adresa: "),
                                     value = address,
                                     input_width=30)
            ...
            if address is not None:
                try:
                    ...
                    run_dialog(Message,
                               _("Oznámení se nezdařilo odeslat:\n")+\
