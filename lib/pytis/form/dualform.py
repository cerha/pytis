...
class SideBrowseDualForm(PostponedSelectionDualForm):
    ...
    def _do_selection(self, row):
        ...
        if focused is None or (isinstance(focused, wx.TextCtrl) and \
            ...
            # None to může být, není-li nic z aplikace zaostřeno.
...
class DescriptiveDualForm(ImmediateSelectionDualForm, Refreshable):
    """Duální formulář s řádkovým seznamem nahoře a formulářem dole.

    Tento formulář slouží k současnému zobrazení přehledu položek a podrobnému
    zobrazení aktuální položky.  Není určen k editaci této položky.
