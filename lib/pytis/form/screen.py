...
class Window(wx.ScrolledWindow, Restorable):
    ...
    """Aktivační konstanta obecného okna."""
...
class Keymap:
    ...
    def define_key(self, key, command, args={}):
        ...
        Argumenty:
          ...
          args -- parametry příkazu, předané při vyvolání příkazu metodě
    ...
    def lookup_key(self, key):
        ...
        Vrací: Je-li na klávesu napojen příkaz, vrať dvojici (COMMAND, ARGS),
          ...
          jako dictionary pro předání metodě 'KeyHandler.on_command()'.
          Je-li na klávesu připojena klávesová mapa (v případě víceklávesových
          definic), je vrácena tato mapa jako instance třídy 'Keymap'.  Není-li
          klávesa definována, vrať 'None'.
...
class KeyHandler:
    ...
    metody 'on_command()'.  Ve třídě se vytvoří klávesová mapa poskládaná
    ...
    def on_key_down(self, event, dont_skip=False):
        ...
        metodu 'on_command()' instance celé aplikace s odpovídajícím příkazem
        jako jejím argumentem.  Pokud takový příkaz neexistuje nebo pokud
        'on_command()' odmítne příkaz zpracovat (vrátí nepravdu), ponech
    ...
    def on_command(self, command, **kwargs):
        ...
        Argumenty:
          ...
          kwargs -- doplňující argumenty 'command'

        Vrací: Pravdu, právě když příkaz byl zpracován a nemají být již
        prováděny další pokusy o jeho zpracování.

        V této třídě metoda nedělá nic a vrací False.

        V potomcích této třídy by metoda měla být předefinována a ošetřovat
        všechny podporované příkazy.  Pokud se metoda k předanému 'command'
        nehlásí, měla by jej, pokud možno, předat metodám stejného názvu svých
        prvků.  Zda je přednostně provedeno vlastní zpracování příkazu, nebo je
        nejprve dána šance vnitřním prvkům, závisí na uvážení dle konkrétní
        třídy.
...
class MenuBar(wx.MenuBar, Restorable):
    """Implementace pull-down menu pomocí tříd 'wxMenuBar' a 'wxMenu'.
    ...
    Menubar je jednotný pro celou aplikaci, avšak některé jeho položky nebo
    části mohou být aktivovány nebo deaktivovány dle kontextu.
                ...
                if isinstance(menu, Menu):
                    ...
                    if k != (None,):
                        ...
                        if keys.has_key(k) and keys[k] != cmd:
                            log(OPERATIONAL,
                                _("Duplicitní klávesa položky menu:"),
                                (k, menu.title(), cmd))
                            log(OPERATIONAL, _("Kolidující příkaz:"), keys[k])
                        else:
                            keys[k] = cmd
            for m in menus:
                check_duplicate_keys(m)
    ...
    def add_menu(self, menu, form=None):
        """Přidej nebo nahraď v menubaru 'menu'.

        Pokud v menubaru již existuje menu se stejným titulkem a se stejnou
        aktivací, bude nahrazeno; pokud je nalezena pouze shoda v titulku,
        bude vloženo vedle menu se shodným názvem,
        jinak je přidáno jako poslední menu.

        Argumenty:

          menu -- instance třídy 'Menu'
    ...
    def activate(self, activations, form=None):
        ...
        Argumenty:
          ...
          activations -- sekvence aktivací a jen těch aktivací, které mají být
            aktivovány; aktivaci 'None' není třeba uvádět, položky s touto
            aktivací jsou aktivní vždy.
          form -- formulář, jehož aktivace se provádí, instance třídy 'Form';
            není-li aktivace spojena s žádným formulářem, tak 'None'
        ...
        for menu in self._menus:
            ...
            # Veškeré nesmyslnosti zde jsou tu záměrně, kvůli wxPythonu.
            if removep:
...
class Menu:
    ...
    def __init__(self, title, items, activation=None):
        ...
        Argumenty:
          ...
          items -- položky menu, sekvence instancí tříd 'Menu' a 'MItem';
            namísto kteréhokoliv prvku může být též funkce vracející instanci
            nebo sekvenci instancí tříd 'Menu' a 'MItem', volaná s jedním
            argumentem, kterým je aktuální instance třídy 'Form', nebo 'None'.
            Je-li alespoň jedna položka funkcí, je menu považováno za dynamické
            (vis Menu.dynamic()).
          activation -- string nebo 'None' určující, v jakém typu obrazovek je
            položka aktivní; je-li 'None', je položka aktivní ve všech
            obrazovkách.  Tento argument je zohledňován pouze v top-level
            menu.
    ...
    def activation(self):
        """Vrať aktivační kategorii položky zadanou v konstruktoru."""
    ...
    def dynamic(self):
        """Vrať pravdu, je-li toto menu dynamické (viz Menu.__init__())."""
...
class MSeparator:
    ...
    místě vyvtořen vizuální oddělovač.
...
class MItem:
    ...
    def __init__(self, title, command, args={}, help='', hotkey=None,
        ...
        Argumenty:
          ...
          command -- instance třídy 'Command' odpovídající příkazu, který má
            být při aktivaci této položky menu vyvolán
          ...
          hotkey -- horká klávesa pro okamžitý výběr položky menu, string nebo
            sekvence stringů dle specifikace v modulu 'command'
          uievent_id -- string identifikující uživatelskou událost generovanou
            před zobrazením položky menu nebo 'None'
    ...
    def title(self):
        """Vrať titulek položky zadaný v konstruktoru."""
        return self._title
    ...
    def uievent_id(self):
        """Vrať identifikátor uživatelské události zadaný v konstruktoru."""
...
class ButtonSpec:
    """Specifikace tlačítka tlačítkové lišty.

    Každé tlačítko je dáno svými atributy popsanými v metodě '__init__()'.

    Třída je čistě specifikační, je považována za immutable a její instance
    tudíž mohou být libovolně sdíleny.
    ...
    def __init__(self, id, label, callback, keytype=KEY_ANY,
        ...
        Argumenty:
          ...
          id -- identifikace tlačítka, neprázdný string prostřednictvím kterého
            se lze později na tlačítko odkazovat
          label -- nápis na tlačítku, neprázdný string
          callback -- funkce, která má být zavolána při stisku tlačítka; funkce
            přijímá jediný argument, kterým je event, jež ji vyvolala
          keytype -- typ klávesy tlačítka, jedna z konstant modulu začínajících
            prefixem 'KEY_'
          activation -- aktivační kategorie tlačítka, jedna z konstant modulu
            začínajících prefixem 'ACT_'

        Typ klávesy ('keytype') nepřímo určuje, jaká klávesa má být tlačítku
        přiřazena.  Typ neříká o jakou klávesu se přímo jedná, nýbrž pouze
        o jaký *typ* klávesy se jedná; konkrétní klávesy jsou jednotlivým typům
        kláves přiřazeny na jediném místě někde jinde.  Vycházíme
        z následujících předpokladů:

        - Budeme mít na různých obrazovkách různá tlačítka s podobnou funkcí,
          kterým by měla být přiřazena tatáž funkční klávesa.

        - Přiřazování tlačítek konkrétním klávesám se bude v průběhu aplikace
          dost často měnit, nechceme tedy klávesu definovat přímo.

        Speciální typ klávesy 'KEY_ANY' říká, že tlačítku má být přiřazena
    ...
    def callback(self):
        """Vrať callback zadaný v konstruktoru."""
        return self._callback

    def keytype(self):
        """Vrať typ klávesy zadaný v konstruktoru."""
        return self._keytype

    def activation(self):
        """Vrať aktivační kategorii zadanou v konstruktoru."""
        return self._activation
...
class ButtonBar(wx.BoxSizer, Restorable):
    ...
    Tlačítková lišta existuje v jediné instanci pro hlavní obrazovku aplikace.
    Tato instance má po svém vytvoření základní sadu tlačítek nezávislých na
    aktuálním obsahu obrazovky.  Celý obsah této lišty však není stálý, mění se
    dle kontextu.  Každá obrazovka má svoji vlastní sadu několika tlačítek,
    které v liště mají být přítomny pouze během práce s touto obrazovkou.
    Proto tato třída poskytuje metody 'add_buttons()' a 'del_buttons()',
    které umožňují danou sadu tlačítek do lišty přidat a pak zase odebrat.
    Konflikty v případě současného přidání tlačítek pro stejný typ klávesy
    zatím nejsou řešeny.

    Požadovaná tlačítka jsou do všech metod předávána jako sekvence instancí
    třídy 'ButtonSpec'.

    """
    def __init__(self, parent, button_specs):
        """Ulož 'parent' a vytvoř tlačítka dle 'button_specs'.

        Argumenty:

          parent -- 'wxFrame', do kterého mají být vkládána vytvářená tlačítka
          button_specs -- sekvence instancí třídy 'ButtonSpec', určující
            tlačítka, která mají být vložena do lišty hned v konstruktoru
    ...
    def _exists(self, id):
        """Vrať pravdu právě když lišta obsahuje tlačítko s 'id'."""
        return self._buttons.has_key(id)

    def _add_button(self, button_spec):
        """Přidej do lišty tlačítko dle 'button_spec'.

        Argumenty:

          button_spec -- instance třídy 'ButtonSpec'
        ...
        # Zatím tlačítka natvrdo aktivujeme, protože máme pouze jedinou
        # aktivační kategorii.
        button.Enable(True)
        self.Layout()
    ...
    def _del_button(self, id):
        ...
        if not self._exists(id):
            return
        # event není potřeba rušit, zmizí spolu s wxButtonem (neověřeno)
    ...
    def add_buttons(self, button_specs):
        """Přidej do lišty tlačítka definovaná 'button_specs'.

        Argumenty:

          button_specs -- sekvence instancí třídy 'ButtonSpec'

        Situace, kdy se některé tlačítko vytvořené na základě 'button_spec'
        dostane do konfliktu s tlačítkem již v liště přítomným, zatím není
        nijak ošetřena.
    ...
    def del_buttons(self, button_specs):
        """Odstraň z lišty tlačítka z 'button_specs'.

        Argumenty:

          button_specs -- sekvence instancí třídy 'Button_Spec'
          
        Tlačítka jsou plně identifikována svými id, jiné srovnávání při
        odstraňování není prováděno.  Pokud některé z předaných tlačítek
        v liště není, je ignorováno.
...
class InfoWindow(object):
    ...
    def __init__(self, title, text, _name='info'):
        ...
        Argumenty:
          ...
          text -- prostý text, který bude zobrazen v okně.  Řádkování i veškeré
            další formátování zůstane nedotčeno (je ponecháno na volající
            straně).
...
class HtmlWindow(InfoWindow):
    ...
    Argument 'text' konstruktoru je zde text s HTML zančkováním.  Text však
    není sám o sobě platným HTML dokumentem.  Neobsahuje hlavičku, ani značky
    <html> a <body>.  Jde jen o zformátovaný text, který bude vsazen do těla
    automaticky vytvořeného dokumentu.
...
class Help:
    """Třída pro specifikaci nápovědy.

    Nápověda je dána svým HTML textem, který je uložen v souboru zadaném
    v konstruktoru.  Podrobněji o parametrech nápovědy viz metoda
    `__init__()'.
    ...
    def __init__(self, file):
        ...
        Argumenty:
          ...
          file -- jméno soubory včetně cesty relativní ke kořenovému adresáři
            nápovědy (TODO: to je který?), jako string
            
        """
        self._file = file

    def file(self):
        """Vrať jméno souboru zadané v konstruktoru jako string."""
        return self._file


class HelpFrame:    
    """Hypertextový frame pro zobrazení nápovědy včetně navigačních prvků.

    Frame je vytvořen na základě specifikace nápovědy instancí třídy 'Help'.

    """
    def __init__(self, parent, spec):
        """Inicializuj frame 'parent' a natáhni text nápovědy.
        
        Argumenty:

           parent -- rodičovské okno
           spec -- specifikace nápovědy, instance třídy 'Help'
...
def wx_html_view(parent, content):
    """Vrať instanci wx widgetu se zobrazeným HTML obsahem.

    Argumenty:

      parent -- rodičovský wx prvek.
