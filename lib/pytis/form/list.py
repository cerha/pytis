...
class ListForm(LookupForm, TitledForm, Refreshable):
    ...
    def _create_grid(self):
        ...
        # Tento fetch je tu kvůli obejití problému s opačným pořadím načítání
        # řádků gridu, které v důsledku vyřazuje DB buffer.
        # Nutno vyřešit jinak...
        self._data.fetchone()
        self._table = table = \
                ...
                if not self._finish_editing(row=row):
                    ...
                    if row < 0 or row >= g.GetNumberRows():
                        ...
                        if not g.IsInSelection(row, 0):
                            ...
                            # Z neznámých důvodů jedno volání nestačí.  Řádek
                            # je vidět jen napůl...  Podruhé už je to lepší :-)
                            g.MakeCellVisible(row, col_)
                            if invoke_callback:
