...
class ListForm(LookupForm, TitledForm, Refreshable):
    ...
    def _on_handled_command(self, command, norefresh=False, **kwargs):
        ...
        # TODO: Příkazy s handlerem by neměly být vůbec používány.  Namísto
        # nich nechť je využíván příkaz COMMAND_CURRENT_ROW_ACTION,
        # kde se handler definuje jako součást argumentů a ne jako argument
        # konstruktoru Command.  Uživatel zkrátka nemá co vytvářet vlastní
        # instance třídy Command...  Tato metoda a příslušná část v on_command
        # jsou zde jen kvůli zpětné kompatibilitě a až se v aplikacích
        # všechy uživatelské příkazy nahradí, bude možné ji zrušit.
    ...
    def _on_current_row_action(self, handler=None, enabled=None,
        ...
        log(EVENT, 'Vyvolávám uživatelský handler akce nad aktuálním řádkem.')
    ...
    def can_current_row_action(self, handler=None, enabled=None,
        ...
        if len(rows) != 1 or rows[0] != self._grid.GetGridCursorRow():
            # Může se stát, že vybraný řádek není aktuální řádek.  Potom by
            # se uživatel mohl domnívat, že pracuje s vybraným, ale ve
            # skutečnosti pracuje s tím, na kterém je kurzor.
            return False
    ...
    def _on_selected_rows_action(self, handler=None, enabled=None,
                             access_groups=None, **kwargs):
        log(EVENT, 'Vyvolávám uživatelský handler akce nad aktuálním výběrem.')
    ...
    def _on_insert_line(self, before=False, copy=False):
        ...
        Argumenty:
          ...
          before -- je-li pravda, nový řádek se vloží před aktuální řádek, jinak
            se vloží za aktuální řádek
        ...
        Vložení nového řádku také není umožněno, pokud mezi právě zobrazenými
        sloupci chybí některý NOT NULL sloupec.
    ...
    def _refresh(self, reset=None, when=None):
        ...
        Argumenty:
          ...
          reset -- určuje, které parametry zobrazení mají být zachovány a které
            ...
            hodnotou z dictionary pro daný klíč.  Parametr 'filter_flag' udává,
...
class SelectRowsForm(CodebookForm):
    ...
    _ACTIVATE_COMMAND_TITLE = _("Použij vybrané řádky")
