...
class ListForm(LookupForm, TitledForm, Refreshable):
    ...
    def _column_context_menu(self, col):
        ...
        items = (M(_("Primární řazení"),
                 ...
                 I(_("Přidat k seskupovacím sloupcům"),
                   command=ListForm.COMMAND_SET_GROUPING_COLUMN,
                   args=dict(col=col)),
                 I(_("Odebrat ze seskupovacích sloupců"),
                   command=ListForm.COMMAND_SET_GROUPING_COLUMN,
                   args=dict(col=col, remove=True)),
                 I(_("Zrušit seskupování úplně"),
                   command=ListForm.COMMAND_SET_GROUPING_COLUMN,
    ...
    def _set_grouping_column(self, col=None, remove=False, reset=False):
        # Možnosti:
        #    - nastavit sloupec jako jediný (zrušit případné ostatní).
        #    - přidat sloupec ke stávajícím
    ...
    def _on_handled_command(self, command, norefresh=False, **kwargs):
        ...
        # nich nechť je využíván příkaz COMMAND_CONTEXT_MENU_ACTION,
    ...
    def _on_context_menu_action(self, handler=None, enabled=None, **kwargs):
        log(EVENT, 'Vyvolávám uživatelský handler akce kontextového menu.')
        if not callable(handler):
            raise ProgramError("Nepřípustný handler akce konetxtového menu:",
                               handler)
        args, kwargs = self._context_menu_handler_args(handler, **kwargs)
        handler(*args, **kwargs)
        #if not norefresh:
