...
class ListForm(LookupForm, TitledForm, Refreshable):
    ...
    """
    # Konvence pro pojmenovávání některých proměnných, zejména v roli argumentů
    # metod:
    # - "row", resp. "col", vždy značí *číslo* řádku, resp. sloupce, počínaje
    #    od 0
    # - řádek dat jako instance Row se pojmenovává "the_row"    
    ...
    class _ListTable(wx.grid.PyGridTableBase):
        ...
        class _EditedRow(_CurrentRow):
            def __init__(self, row, the_row, fieldspec, data, new=False,
                ...
                if __debug__ and config.server:
                    import pytis.remote
                # TODO: nevím, pro není pytis.data.Row, i když je nahoře import
        ...
        def _retrieve_row(self, row):
            def fetch(row, direction=pytis.data.FORWARD):
                data = self._data
                # Následující manévr vychází z předpokladu, že zpětný chod
                # bude pokračovat i nadále směrem zpět.  Proto se snažíme
                # naznačit cachi, že má stáhnout data předchozí, ne
                # následující.
                #
                # TODO: manévr nefunguje, pokud máme např. 4 řádky v gridu                
        ...
        def GetValue(self, row, col, inputfield=False):
            ...
            if not inputfield and \
               ...
               value == 'F':
                # V této podobě gridu je 0 považována (z neznámých důvodů,
                # možná to souvisí s C++ přijímajícím zde pouze strings) za
                # pravdu.
                value = ''
            return value
...

    ...
    """UI test na editaci řádku."""
    UI_EXISTS_CONDITION = 'UI_EXISTS_CONDITION'
    """UI test na přítomnost vyhledávací podmínky."""
    UI_SORTING = 'UI_SORTING'
    """UI test na zapnuté třídění."""
    ...
    def _create_grid(self, data_init=True,
        ...
        log(DEBUG, 'Vytváření nového gridu',
        ...
        # Společná inicializace create a recreate
        self._create_recreate_grid(g)
        # Hotovo
        log(DEBUG, 'Nový grid vytvořen')
        return g
    ...
    def current_key(self):
        ...
        Vrací: Sekvenci instancí třídy 'pytis.data.Value' nebo 'None'.
    ...
    def on_data_change(self):
        ...
        dobu.  K přímým updatům slouží metody 'reset()', 'reshuffle()' a
        'refresh()'.
    ...
    def _on_handled_command(self, command, **kwargs):
        ...
        # Zjistíme, zda má uživatelský handler klíčové argumenty
        import inspect
...
#                    prefill[k] = the_row[k].value()
    ...
    def _on_delete_line(self):
        log(EVENT, 'Pokus o smazání řádku')
        if not self._data.accessible(None, pytis.data.Permission.DELETE):
            message('Nemáte přístupová práva pro mazání záznamů v této ' + \
                    'tabulce!', beep_=True)
            return False
        if not self.editable:
            message('Needitovatelná tabulka!', beep_=True)
        ...
        if success:
           message('Řádek smazán:', ACTION, key or condition)
           r = self._current_row()
    ...
    def total_width(self):
        """Vrať součet šířek všech sloupečků v pixelech."""
    ...
    def refresh(self, reset=None, when=None):
        ...
        if delay:
            log(DEBUG, 'Refresh odložen do ukončení editace')
            return False
    ...
    def Show(self, show, refresh=True):
        # Zde implicitně voláme refresh, protože to ušetří spoustu potenciálních
        # potíží s aktualizací.  Pokud toto není žádoucí, lze využít klíčový
...
class CodeBook(ListForm, PopupForm, KeyHandler):
    ...
    def __init__(self, parent, *args, **kwargs):
        parent = self._popup_frame(parent, 'Číselník')
    ...
    def _init_attributes(self, ctype=None, begin_search=False, **kwargs):
        ...
        Argumenty:
          ...
          ctype -- instance třídy 'pytis.data.Codebook', odpovídající typu
            číselníku, pro který je formulář vyvolán
...
class BrowseForm(ListForm):
    ...
    def _context_menu(self):
        ...
        menu = (
            ...
            # TODO:uievent_id= neaktivní, když už jsem v DescriptiveDualFormu
