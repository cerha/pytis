...
class InputField(object, KeyHandler, CallbackHandler):
    ...
    """Povolení události, pouze pokud je obsah políčka změněn."""
    UI_IS_ENABLED =  'UI_IS_ENABLED'
    """Povolení události, pouze pokud je políčko editovatelné."""

    CALL_LEAVE_FIELD = 'CALL_LEAVE_FIELD'
    """Callback volaný při požadavku na opuštění políčka. Bez argumentů."""
    CALL_COMMIT_FIELD = 'CALL_COMMIT_FIELD'
    """Voláno při opuštění políčka s potvrzením hodnoty. Bez argumentů."""
    CALL_FIELD_CHANGE = 'CALL_FIELD_CHANGE'
    """Callback volaný při změně políčka, pokud je nová hodnota validní.

    Argumenty:

      fid -- id políčka, string.
      value -- nová hodnota políčka, instance 'pytis.data.Value'.
    
    """
    CALL_SKIP_NAVIGATION = 'CALL_SKIP_NAVIGATION'
    """Voláno, pokud má být nějaký ui prvek přeskočen při navigaci.
    ...
    def __init__(self, parent, fspec, data, guardian=None, inline=False,
                 accessible=True):
        """Vytvoř vstupní políčko, podle specifikace a typu dat.

        Argumenty:

          parent -- libovolná instance 'wx.Window', která má být používána
            jako wx rodič všech vytvářených wx prvků
          fspec -- specifikace prezentačních vlastností, instance třídy
            'spec.FieldSpec'
    ...
    def _menu(self):
        ...
        return (MItem("Vrátit původní hodnotu",
    ...
    def show_popup_menu(self, position=None):
        """Zobraz kontextové menu vstupního políčka.

        Argumenty:
        
          position -- tuple (x, y) udávající pozici relativně k vstupnímu
            prvku tohoto políčka.
    ...
    def _validation_error_handler(self):
        # Ošetři validační chybu.
        
        # Tato metoda je volána, pokud uživatel opouští políčko, ve kterém
        # změnil hodnotu, ale nová hodnota není validní.  Vrací pravdu pokud
        # byla situace nějak ošetřena, nebo nepravdu, pokud má být situace
        # ošetřena standardním způsobem (zobrazením chybové zprávy ve stavovém
        # řádku).
        
        # V této třídě nedělá nic, ale odvozená třída může předefinováním
        # vyvolat například nějakou interaktivní akci apod.
    ...
    def _on_change(self, event=None):
        ...
        Odvozené třídy by tuto metodu neměly předefinovávat. Nechť raději
        předefiují metodu '_on_change_hook()'. Každá třída by měla zabezpečit
        ošetření editačních událostí voláním této metody.
    ...
    def reset(self):
        ...
        """
        log(DEBUG, 'Reset hodnoty políčka', self.id())
...
class EnumerationField(InputField):
    ...
    množiny hodnot. Množinu hodnot určuje datový typ samotný (viz metoda
    'pytis.data.Codebook.values()').

    Protože pracujeme s datovým typem 'pytis.data.Codebook', jsou vstupními a
    výstupními hodnotami metod 'get_value()' a '_set_value()' sekvence, neboť
    datový typ může potenciálně pracovat i se sloupečky s vícenásobnou vazbou,
    takže pro jistotu používá sekvence vždy - i pokud jsou jednoprvkové.
    ...
    """
    def _values(self):
        """Vrať list všech hodnot daných datovým typem."""
        assert isinstance(self._type, pytis.data.xtypes.Codebook)
        return map(lambda v: v[0], self._type.values())

    def get_value(self):
        """Vrať hodnotu políčka jako jednoprvkový tuple obsahující string."""
    ...
    def _set_value(self, value):
        ...
        Argumenty:
            ...
            value -- hodnota políčka, jednoprvkový tuple obsahující string nebo
              přímo string
        ...
        # TODO: Tento test nefunguje pro políčka, která vzniknou z Codebooků
...
class Invocable:
    """Poděděním této třídy lze přidat vlastnost vyvolání výběru.

    Abstraktní třída pro políčka, která umožňují vyvolat pro výběr hodnoty
    nějakou akci (většinou v podobě modálního popup okna).

    Třída, která dědí z této třídy musí být zárověň potomkem jiné třídy, která
    definuje metodu '_create_widget()' (nebo ji definovat sama). Tato metoda
    bude použita k vytvoření ovládacího prvku a navíc bude k tomuto ovládacímu
    prvku přibaleno tlačítko pro vyvolání výběru.

    Výběr lze vyvolat také klávesou příkazu
    'Invocable.COMMAND_INVOKE_SELECTION', pokud základní třída vytvářející
    ovládací prvek použije metodu '_add_key_handler()' (což by měla vždy).
    ...
    def _on_invoke_selection(self, **kwargs):
        """Callback pro akci vyvolání výběru."""
        raise ProgramError("This method must be overriden!")
...
class DateField(Invocable, TextField):
    ...
    def _on_invoke_selection(self, **kwargs):
        """Zobraz kalendář a po jeho skončení nastav hodnotu políčka."""
...
class CodeBookField(Invocable, TextField):
    ...
    Bude použito v případě, že datový sloupeček je typu 'pytis.data.Codebook' a
    prezentační specifikace políčka definuje navázaný číselník
    (viz. 'FieldSpec.codebook()').

    Tento typ vstupního pole pracuje s vícehodnotovými políčky, takže hodnoty
    jsou sekvencemi (viz. 'MultiTextField') a to i v případě, že jsou tyto
    sekvence jednoprvkové.
       
    Jako akci pro vyvolání výběru definuje zobrazení formuláře 'list.CodeBook'.
    Název specifikace číselníku a další vlastnosti jsou dány specifikátorem
    'codebook' ve specifikaci políčka (instance 'CodeBookSpec').

    K políčku může být volitelně přidružen displej, který je určen k zobrazení
    popisu z číselníku k vybrané hodnotě.  Specifikace displeje je také
    součástí 'CodeBookSpec'.
    ...
    def _validation_error_handler(self):
        ...
        if error:
            return False
        msg = _("Číselník neobsahuje hodnotu %s") % value +"\n"+ \
              _("Chcete do číselníku přidat nový záznam?")
        if (run_dialog(Question, msg)):
            prefill = {cbspec.returned_column(): value}
            result = run_form(PopupEditForm, cbspec.name(), prefill=prefill)
            #TODO: Update datového objektu číselníku?
            self._on_change_hook()
...
class ListField(InputField):
    """Seznamové políčko pro zobrazení záznamů ze závislé tabulky

    Políčko slouží k zobrazení množniny záznamů z jiné tabulky (datového
    objektu) vztahujících se k nastavené vnitřní hodnotě políčka (viz také
    metoda '_set_value()').  Typické využití je při vazbě přes cyzí klíč v
    relační databázi.

    Vzhled a chování je určeno specifikátorem 'references' ve specifikaci
    políčka (viz. 'pytis.form.FieldSpec').  Datový objekt navázané tabulky je
    získán pomocí resolveru podle jména určeného specifikací. 
    ...
    def _create_ctrl(self):
        ...
        # Specifikace sloupců uložím do dictionary.
        colspec = {}
    ...
    def _choose_returned_column(self, column):
        """Vrať hodnotu vybraného sloupce."""        
    ...
    def _enable(self):
        pass # ListControl se stejně nedá nijak editovat
    
    def _disable(self, change_appearance):
        pass # ListControl se stejně nedá nijak editovat

    def get_item(self):
        """Vrať aktuální hodnotu vybraného políčka."""
    ...
    def _set_value(self, value):
        """Nastav vnitřní hodnotu políčka a přenačti seznam závislých záznamů.
        
        Každé nastavení hodnotu vyvolá výběr z navázaného datového objektu.
        Výběr vyfiltruje z navázané tabulky ty záznamy jejichž sloupeček určený
        specifikací 'references' (instance 'RefSpec') jako 'key' se svou
        hodnotou shoduje s hodnotou předanou argumentem 'value'.
    ...
    def on_command(self, command, **kwargs):
        if command == self.COMMAND_INVOKE_EDIT_FORM:
            ...
            if isinstance(current_form(), PopupForm):
                run_dialog(Warning, _("Celou tabulku nemůžete zobrazit, " + \
                                   "pokud je otevřeno modální okno formuláře!"))
                return True
...
class HiddenField(InputField):
    """Skryté (virtuální) políčko. Políčko pouze drží nastavenou hodnotu."""
    ...
    def get_value(self):
        """Vrať dříve uloženou hodnotu políčka."""
        try:
            return self._value
        except:
            return None
