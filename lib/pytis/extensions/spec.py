...
def cb_computer(codebook_field, column, default=None):
    ...
    Argumenty:
      ...
      codebook_field -- id číselníkového políčka, z jehož číselníku má být
        hodnota převzata.

      column -- id sloupce v datovém objektu číselníku, jehož hodnota má být
        dopočítávací funkcí vrácena.
        
      default -- implicitní hodnota použitá pokud je hodnota zvoleného sloupce
        v číselníku nedefinována.
...
def cb_computed_field_args(codebook_field, column, default=None, virtual=False):
    """Vrať slovník argumentů pro FieldSpec políčka dopočítávaného z číselníku.
    ...
    virtuální políčko (k němuž neexistuje datový sloupec), a k vráceným
    argumentům bude automaticky přidán také argument 'type_'.  Jako typ je však
    vždy doplňen typ 'pytis.data.String()', takže pokud je sloupec ve
    skutečnosti jiného typu, je nutné argument 'type_' definovat explicitně.
    Ve většině případů však toto zjednodušení postačuje.

    Vrací slovník argumentů pro 'FieldSpec' políčka.
    ...
    Pokud bychom přidali ještě 'virtual=True', dostaneme:
