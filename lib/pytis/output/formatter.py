...
"""Velikost levého okraje stránky, instance třídy 'Unit'."""
...
class LoutFormatter(Tmpdir):
    ...
    def _format_list(self, template, stream, indent):
        ...
        try:
            ...
            raise TemplateException(_("Neznámá značka uvození seznamu"),
    ...
    def _format_group(self, template, stream, indent):
        ...
        if balance:
            if len(balance) != nelements:
                raise TemplateException(_("Balance neodpovídá počtu prvků"),
                                    balance, template)
            sum = reduce(operator.__add__, balance)
            if sum <= 0 or some(lambda x: x < 0, balance):
                raise TemplateException(_("Chybné číslo v balanci"),
                                    balance, template)
            bk = 1.0 / sum
    ...
    def _format_table(self, template, stream, indent):
        ...
        def cell_alignment(alignment):
            if alignment == Table.Column.ALIGN_LEFT:
                ...
                raise TemplateException(_("Neznámý druh zarovnání sloupce"),
                                        c.alignment)
            return break_
    ...
    def _format_image(self, template, stream, indent):
        ...
        if tfile_name == '..' or starts_with(tfile_name, '../'):
            raise TemplateException(_("Nikoliv bezpečné jméno souboru"),
                                    template.file_name())
        import config
    ...
    def _format(self, template, stream, indent=''):
        if is_anystring(template):
            ...
            try:
                ...
                raise TemplateException(_("Chybný element"), template)
            try:
                processor = self._mark_processors[tclass]
            except KeyError:
                raise TemplateException(_("Neznámý element"), template)
