...
class ResolverSpecError(ResolverError):
    """Výjimka vyvolaná není-li ve specifikačním modulu žádaná specifikace."""
...
class Resolver(object):
    ...
    Je-li nastavena konfigurační volba 'config.auto_reload_defs' na pravdu,
    resolver kontroluje před každým vrácením specifikace její modul na změny,
    v opačném případě je po prvním získání specifikace tato pod daným jménem
    vracena již neustále, bez ohledu na případné změny jejího modulu.

    """
    def __init__(self):
        """Inicializuj resolver."""
        # TODO: Validace by měla být prováděna dle času poslední modifikace
        # souboru nebo něčeho takového.
        import config
