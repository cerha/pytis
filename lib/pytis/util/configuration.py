...
class Configuration:
    ...
    class Option(object):
        ...
        předdefinovaných tříd '_StringOption', '_NumericOption' nebo
        '_BooleanOption' (viz níže).
        ...
        def init_value(self, force=False):
            ...
            if force or self._value is self._undefined:
                ...
                self._value = value
                # Callable objekty nelze kopírovat, tak zkopírujeme alespoň
                # všechny ostatní...
                if callable(value):
        ...
        def description(self, full=False):
            """Vrať popis volby 'name' jako řetězec.
        
            Pokud je argument `full' pravdivý vrácen popis včetně případné
            podrobnějši dokumentace.  Řetězec tak může být víceřádkový a délka
            jednoho řádku může přesahovat 80 znaků.
            
            Pokud je `full' nepravda, vrací pouze stručný jednořádkový popis.
    ...
    class _FileOption(_StringOption):
    ...
    class _CommandlineOption(Option):
        """Mix-in třída pro volby příkazové řádky."""
        _CMDLINE = True
        
    # Volba pro konfiguraci samu

    class _Option_config_file(_StringOption, _HiddenOption):
    ...
    class _Option_user_config_file(_StringOption, _HiddenOption):
    ...
    class _Option_help(_BooleanOption, _CommandlineOption, _HiddenOption):
        _DESCR = _("Volba odpovídající --help na příkazové řádce.")
        _DEFAULT = False
        
    class _Option_debug(_BooleanOption, _CommandlineOption):
    ...
    class _Option_debug_on_error(_BooleanOption, _CommandlineOption):
    ...
    class _Option_debug_memory(_BooleanOption, _CommandlineOption):
    ...
    class _Option_auto_reload_defs(_BooleanOption):
        _DESCR = _("Příznak automatického přenačítání změněných definičních "
                   "souborů.")
        _DOC = _("Je-li zapnut, je zaručeno přenačtení definičních souborů "
                 "aplikace v případě jejich změny.  Někdy to může zpomalovat "
                 "běh aplikace.  Implicitně má tato volba stejnou hodnotu jako "
                 "volba 'debug'.")
        def default(self):
            return self._configuration.debug

    class _Option_test_run_interactive(_BooleanOption, _HiddenOption):
        _DESCR = _("Příznak určující, zda mají být spouštěny i interaktivní "
                   "testy.")
        _DOC = _("Týká se pouze regresivního testování.")

    class _Option_custom_debug(Option, _HiddenOption):
        _DESCR = _("Zvláštní ladící funkce, napojená na příkaz "
                   "'COMMAND_CUSTOM_DEBUG'.")
        _DEFAULT = (lambda: None)

    # Cesty a adresáře

    class _Option_def_dir(_FileOption, _CommandlineOption):
    ...
    class _Option_doc_dir(_FileOption, _CommandlineOption):
        _DESCR = _("Adresář obsahující dokumentační soubory.")
    ...
    class _Option_dbuser(_StringOption, _CommandlineOption):
        _DESCR = _("Uživatelské jméno (login) pro databázové spojení.")
    ...
    class _Option_dblisten(_BooleanOption):
    ...
    class _Option_log_one_line_preferred(_BooleanOption):
    ...
    class _Option_log_module_filter(_StringOption):
    ...
    class _Option_printing_command(_StringOption):
    ...
    class _Option_sendmail_command(_StringOption):
        _DESCR = _("Shellový příkaz sendmail včetně celé cesty.")
        _DEFAULT = '/usr/lib/sendmail'
        
    # Ostatní konfigurační volby

    class _Option_application_name(_StringOption):
    ...
    class _Option_date_time_format(_StringOption):
    ...
    class _Option_initial_fetch_size(_NumericOption):
        _DESCR = _("Počet řádků, které se přednačtou do cache při prvním "
                   "selectu z datového objektu.")
        _DEFAULT = 100

    class _Option_fetch_size(_NumericOption):
    ...
    class _Option_show_tooltips(_BooleanOption):
        _DESCR = _("Příznak zobrazování bublinové nápovědy.")
        _DEFAULT = True
    ...
    class _Option_show_splash(_BooleanOption):
        _DESCR = _("Příznak zobrazování úvodního uvítacího dialogu.")
        _DEFAULT = True
        
    class _Option_cache_spec_onstart(_BooleanOption):
        _DESCR = _("Příznak cachování specifikací při startu aplikace.")
        _DEFAULT = True

    class _Option_startup_forms(_StringOption, _CommandlineOption):
        _DESCR = _("Seznam formulářů, které mají být otevřeny po spuštění "
                   "aplikace.")
        _DEFAULT = None

    class _Option_form_state(Option, _HiddenOption):
        _DESCR = _("Tato volba je využívána pro ukládání stavu formulářů.")
        _DEFAULT = {}

    class _Option_application_state(Option, _HiddenOption):
        _DESCR = _("Tato volba je využívána pro ukládání stavu aplikace.")
        _DEFAULT = {}
    ...
    def options(self):
        """Vrať seznam názvů všech konfiguračních voleb jako tuple řetězců.""" 
    ...
    def type(self, name):
        """Vrať datový typ volby 'name' jako instanci 'pytis.data.Type'.""" 
        return self._options[name].type()

    def changed(self, name):
        """Vrať pravdu, pokud hodnota volby 'name' byla změněna aplikací.
