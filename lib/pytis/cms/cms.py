...
class Languages(Specification):
    title = _(u"Jazyky")
    help = _(u"Správa jazyků dostupných v CMS.")
    table = 'cms_languages'
...
class Modules(Specification):
    title = _(u"Moduly")
    help = _(u"Správa rozšiřujících modulů použitelných ve stránkách.")
    table = 'cms_modules'
    ...
    def bindings(self):
        return (Binding('actions', _(u"Dostupné akce tohoto modulu"), self._spec_name('Actions'),
                        'mod_id'),)
    def actions(self):
        return (Action('reload', _(u"Přenačíst dostupné akce"), self._reload_actions),)
    def on_delete_record(self, record):
        ...
        data.close()
        if count:
            return _(u"Modul je používán v existujících stránkách.\n"
    ...
    def _reload_actions(self, record):
        ...
        if module:
            ...
            result = run_dialog(CheckListDialog, title=_(u"Nalezené akce"),
                                message=_(u"Zaškrtněte akce, které chcete zpřístupnit webovým "
                                          "uživatelům:"),
...
class Menu(Specification):
    title = _(u"Menu")
    help = _(u"Správa položek hlavního menu, jejich hierarchie a obsahu.")
    table = 'cms_menu'
    ...
    def fields(self): return (
        ...
        Field('identifier', _(u"Identifikátor"), width=20, fixed=True, editable=ONCE,
              ...
              descr=_(u"Identifikátor bude vystupovat jako vnější adresa stránky.  Může být použit "
        ...
        Field('title', _(u"Název"), width=20, not_null=True, maxlen=32,
              descr=_(u"Název položky menu - krátký a výstižný.")),
        Field('heading', _(u"Nadpis"), width=32, maxlen=40,
              descr=_(u"Hlavní nadpis při zobrazení stránky.  Pokud ponecháte nevyplněné, "
        ...
        Field('description', _(u"Popis"), width=72,
              descr=_(u"Stručný popis stránky (zobrazen v menu jako tooltip).")),
        Field('content', _(u"Obsah"), type=pd.StructuredText(), compact=True, height=20, width=80,
              descr=_(u"Text stránky formátovaný jako LCG strukturovaný text (wiki)")),
        Field('mod_id', _(u"Modul"), not_null=False,
              codebook=self._spec_name('Modules', False), allow_codebook_insert=True,
              descr=_(u"Vyberte rozšiřující modul zobrazený uvnitř stránky.  Ponechte prázdné pro "
                      "prostou textovou stránku.")),
        Field('modname', _(u"Modul")),
        Field('parent', _(u"Nadřízená položka"), not_null=False,
              codebook=self._spec_name('MenuParents', False), value_column='menu_item_id',
              runtime_filter=computer(self._parent_filter),
              descr=_(u"Vyberte bezprostředně nadřízenou položku v hierarchii menu.  Ponechte "
                      "prázdné pro stránky na nejvyšší úrovni menu.")),
        Field('published', _(u"Zveřejněno"), width=6, default=True, fixed=True, 
              descr=_(u"Umožňuje řídit dostupnost dané položky nezávisle pro kažou jazykovou "
                      "verzi.")),
        Field('ord', _(u"Pořadí"), width=8, editable=ALWAYS, fixed=True, 
              descr=_(u"Zadejte číslo určující pořadí položky v menu (mezi stránkami na stejné "
    ...
    def check(self, record):
        ...
        if record['parent'].value() is not None:
            if record['parent'].value() == record['menu_item_id'].value():
                return ('parent', _(u"Položka nemůže být nadřízená sama sobě."))
            if not record.new():
                ...
                data.close()
                if count:
                    return ('parent', _(u"Nelze přiřadit podřízenou položku jako nadřízenou "
                                        "(cyklus v hierarchii)."))
        count = data.select(condition=pd.AND(*self._check_menu_order_condition(record)))
        data.close()
        if count:
            return ('ord', _(u"Stejné pořadové číslo už má jiná položka na této úrovni menu."))
    ...
    def on_delete_record(self, record):
        ...
        data.close()
        if count:
            return _(u"Položka má podřízené položky.\n"
    ...
    layout = (VGroup('identifier', 'parent', 'mod_id', 'ord',
                     label=_(u"Globální vlastnosti (společné pro všechny jazyky)")),
    ...
    def bindings(self):
        return (
            Binding('rights', _(u"Přístupová práva"), self._spec_name('Rights'),
...
class Users(Specification):
    title = _(u"Uživatelé")
    help = _(u"Správa uživatelských účtů, které je poté možno zařazovat do rolí.")
    table = 'cms_users'
    def fields(self): return (
        Field('uid', _(u"UID"), width=5, default=nextval('cms_users_uid_seq')),
        Field('login', _(u"Přihlašovací jméno"), width=16),
    ...
    sorting = (('login', ASC),)
    def bindings(self):
        return (Binding('roles', _(u"Uživatelské role"), self._spec_name('UserRoles'), 'uid'),
                Binding('sessions', _(u"Historie přihlášení"), self._spec_name('UserSessionLog'),
                        'uid'),
                )


class Roles(Specification):
    title = _(u"Uživatelské role")
    help = _(u"Správa dostupných uživatelských rolí, které je možné přiřazovat uživatelům.")
    table = 'cms_roles'
    def fields(self): return (
    ...
    def bindings(self): return (
        Binding('users', _(u"Uživatelé zařazení do této role"), self._spec_name('RoleUsers'),
                'role_id'),
        )
...
class SystemRoles(Roles):
    ...
    help = _(u"Systémové role jsou uživatelům přiřazeny automaticky.")
...
class AllRoles(Roles):
    title = _(u"Role")
    help = _(u"Role dostupné pro přiřazování práv akcím (obsahuje systémové i uživatelsky definované role).")
    condition = None

    
class UserRoles(Specification):
    title = _(u"Přiřazení uživatelských rolí")
    help = _(u"Správa přiřazení uživatelských rolí jednotlivým uživatelům.")
...
class RoleUsers(UserRoles):
    help = _(u"Správa přiřazení uživatelů jednotlivým uživatelským rolím.")
    layout = ('uid',)
    columns = ('uid', 'login', 'fullname')
...
class Actions(Specification):
    ...
    help = _(u"Výčet podporovaných akcí pro jednotlivé moduly.")
    table = 'cms_actions'
    access_rights = pd.AccessRights((None, ('cms_admin', pd.Permission.ALL)),
                                    (None, ('cms_user', pd.Permission.VIEW)))
    def fields(self): return (
...
class GenericActions(Actions):
    title = _(u"Akce společné pro všechny položky menu")
    help = _(u"Výčet podporovaných akcí společných pro všechny položky menu.")
...
class Rights(Specification):
    title = _(u"Přístupová práva")
    help = _(u"Přiřazení práv k akcím modulů pro jednotlivé uživatelské role.")
    table = 'cms_rights'
    def fields(self): return (
...
class _Log(Specification):
    ...
    def _hostname(self, row, ip_address):
        try:
            ...
            hostname = _(u"Neznámý")
        return hostname
        
class SessionLog(_Log):
    title = _(u"Log přihlášení")
    help = _(u"Záznam informací o přihlášení uživatelů k webu.")
    ...
    def fields(self): return (
        ...
        Field('success', _(u"Úspěch"), width=3),
        Field('start_time', _(u"Začátek"), width=17),
...
class AccessLog(_Log):
    title = _(u"Log přístupů")
    help = _(u"Záznam informací o přístupu uživatelů k jednotlivým stránkám/modulům webu.")
    table = 'cms_access_log_data'
    public = True
    def fields(self): return (
        Field('log_id'),
        Field('timestamp', _(u"Datum a čas"), width=17),
        Field('uri', _(u"Cesta"), width=17),
        Field('uid', _(u"Uživatel"), codebook=self._spec_name('Users')),
...
class Themes(Specification):
    ...
    help = _(u"Správa barevných motivů.")
    table = 'cms_themes'
    fields = (
        Field('theme_id'),
        ...
        _Field('highlight-bg', _(u"Zvýrazněné pozadí"),
               descr=_(u"Zvýraznění pomocí barvy pozadí může být použito na různých místech, "
                       "např. pro odlišení aktuální položky menu, aktuálního jazyka, nalezeného "
                       "záznamu ve formuláři apod.")),
        _Field('link', _(u"Odkaz"),
               descr=_(u"Barva textu nenavštíveného odkazu.")),
        _Field('link-visited', _(u"Navštívený odkaz"),
               descr=_(u"Barva textu navštíveného odkazu.  Pokud ponecháte prázdné, bude "
                       "použita stejná barva jako u nenavštíveného odkazu.")),
        _Field('link-hover', _(u"Aktivní odkaz"),
               descr=_(u"Obarvení odkazu při při pohybu myší nad ním.  Pokud ponecháte "
                       "prázdné, nebude odkaz na pohyb myši reagovat.")),
        _Field('border', _(u"Orámování")),
        _Field('heading-fg', _(u"Text"),
               descr=_(u"Barvy nadpisů jsou používány pro zvýrazenění názvů kapitol, panelů a "
        ...
        _Field('heading-bg', _(u"Pozadí"),
               descr=_(u"Barva pozadí pro nadpisy, které používají zvýraznění barvou pozadí."
                       "Typicky jsou to záhlaví částí stránky (menu, panely apod.) a nadpisy "
                       "vyšších úrovní.")),
        _Field('heading-line', _(u"Podtržení"),
               descr=_(u"Barva podtržení pro nadpisy, které používají zvýraznění podtržením."
                       "Typicky jsou to nadpisy nižších úrovní.")),
        _Field('frame-fg', _(u"Text"),
               descr=_(u"Rámy jsou obecně využívány pro odlišení samostatných prvků stránky, "
        ...
        _Field('frame-bg', _(u"Pozadí"),
               descr=_(u"Barva pozadí rámů pro jejich vizuální odlišení od ostatního obsahu "
                       "stránky.")),
        _Field('frame-border', _(u"Orámování"),
               descr=_(u"Barva orámování okrajů plochy rámu na přechodu mezi barvou pozadí rámu "
                       "a jeho podkladem.")),
        _Field('top-fg', _(u"Text"),
               descr=_(u"Na podkladových plochách se většinou text přímo nevyskytuje, ale "
                       "některé styly to mohou předepisovat.")),
        _Field('top-bg', _(u"Pozadí"),
               descr=_(u"Podkladové plochy tvoří okolí hlavního obsahu stránky a panelů.")),
        _Field('top-border', _(u"Orámování"),
               descr=_(u"Určuje barvu rámečku na přechodu mezi podkladovou plochou a obsahovými "
                       "částmi stránky")),
        ...
        _Field('meta-bg', _(u"Pozadí"),
               descr=_(u"Odlišení informačního řádku v detailních výpisech, jako např. "
                       "informace o datu a autorovi v seznamu článků")),
        _Field('table-cell', _(u"Standardní pozadí buňky")),
        _Field('table-cell2', _(u"Stínované pozadí buňky")),
        _Field('help', _(u"Nápověda formuláře")),
        _Field('inactive-folder', _(u"Neaktivní záložka")),
        )
