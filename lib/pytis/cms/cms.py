...
class Modules(Specification):
    ...
    def actions(self):
        return (Action('a_prenacist_dostupne_akce', _("Přenačíst dostupné akce"), self._reload_actions),)
...
class Menu(Specification):
    ...
    def bindings(self):
        return (Binding('b_pristupova_prava', _("Přístupová práva"), self._spec_name('Rights'),
...
class Users(Specification):
    ...
    sorting = (('login', ASC),)
    def bindings(self):
        return (Binding('b_uzivatelske_role', _("Uživatelské role"), self._spec_name('UserRoles'), 'uid'),
                Binding('b_historie_prihlaseni', _("Historie přihlášení"), self._spec_name('UserSessionLog'), 'uid'),
                )
...
class Roles(Specification):
    ...
    def bindings(self): return (
        Binding('b_uzivatele_zarazeni_do_teto_role', _("Uživatelé zařazení do této role"), self._spec_name('RoleUsers'), 'role_id'),
        )
