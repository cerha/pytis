...
./tools/gsql.py pytis.dbdefs.db_pytis_crypto > /tmp/pytis-crypto.sql
...
prostředí pomocí =pytest pytis/demo/=
...
Soubor .github/workflows/tests.yml definuje prostředí pro automatické spouštění
...
Potom mohu v adresáři pytis, kde je .github/workflows/tests.yml, spustit zde
...
Užitečné argumenty:
  ...
  --reuse   ... restartuj stávající kontejner, pokud existuje (vynechají
                některé kroky jeho inicializace, např instalace balíků).
  -j job    ... spusť jen vybraný job (v našem případě 'test' nebo 'test-py27')
  -W soubor ... spusť daný workflow (výchozí je .github/workflows/tests.yml)
  -b        ... namountuj filesystém přímo z hostitelského systému. Výchozí
                chování je pouze vytvoření snapshotu adresáře, ve kterém se
                případné změny neprojevují.  Hodí se zejména při spouštění
                příkazů přímo v běžícím kontejneru (viz dále).
  --container-architecture linux/amd64 ... potřebné pokud nespouštíme na Linuxu
                (lepší zapsat do .actrc na samostatný řádek).

Do běžícího kontejneru (pokud běh neskonší úspěšně) se potom lze přihlásit.
Nejprve si vypíšu seznam běžících kontejnerů pomocí 'docker ps' a potom zavolám
...
podle definice env v tests.yml, takže je nutné si ho nastavit ručně:
...
pytest pytis/presentation/spec.py
