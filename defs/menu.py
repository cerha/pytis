...
class ApplicationRoles(ApplicationRolesSpecification):
    ...
    bindings = (pytis.presentation.Binding(_("Obsahuje role"), 'menu.ApplicationRolesMembers', id='members',
                ...
                pytis.presentation.Binding(_("Rozšířený náhled menu"), 'menu.ApplicationRoleMenuExtended', id='extmenu',
                                           arguments=(lambda row: dict(roleid=row['roleid']))),
                pytis.presentation.Binding(_("Rozšířený náhled chystaného menu"), 'menu.ApplicationPreviewRoleMenuExtended', id='previewextmenu',
                                           arguments=(lambda row: dict(roleid=row['roleid'],
                                                                       new=pytis.data.Value(pytis.data.Boolean(), True)))),
                )
...
class ApplicationMenuM(pytis.presentation.Specification):
    ...
    bindings = (pytis.presentation.Binding(_("Rozpis práv podle rolí"), 'menu.ApplicationMenuRightsFoldable', id='role_rights',
                ...
                pytis.presentation.Binding(_("Rozpis práv podle sloupců"), 'menu.ApplicationMenuRightsFoldableColumn', id='column_rights',
                                           arguments=(lambda row: dict(shortname=row['shortname'],
                ...
                pytis.presentation.Binding(_("Chystaná práva položky"), 'menu.ApplicationPreviewRights', id='preview_rights',
                                           arguments=(lambda row: dict(shortname=row['shortname'],
                                                                       new=pytis.data.Value(pytis.data.Boolean(), True,),
                                                                       multirights=ApplicationMenuM._multiform_row(row),
                                                                       ))),
    ...
    def actions(self): return (
        ...
        pytis.presentation.Action(_("Odstranit nadbytečná práva"), self._remove_redundant,
    ...
    def _multiaction_warning(self):
        ...
        if count > 1:
            pytis.form.run_dialog(pytis.form.Warning,
                                  _("Pozor, tato položka se v menu vyskytuje na více místech."))
    def _row_editable(self, row):
