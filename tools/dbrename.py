...
def run(current_db, sample_db):
    ...
    # Triggers
    sample_c.execute("select tgname, nspname, relname, tgtype "
                     "from pg_trigger join pg_class on tgrelid=pg_class.oid "
                     "join pg_namespace on relnamespace=pg_namespace.oid "
                     "where not tgisinternal and (tgname like (nspname||'\\_\\_%'))")
    ...
    for row in sample_c.fetchall():
        ...
        if match:
            ...
            current_c.execute("select tgname from pg_trigger "
                              "join pg_class on tgrelid=pg_class.oid "
                              "join pg_namespace on relnamespace=pg_namespace.oid "
                              "where not tgisinternal and (tgname=%s or tgname=%s) "
                              "and nspname=%s and relname=%s and tgtype=%s", (old_name,) + row)
            ...
            if count == 0:
                ...
                if cur_name != name:
                    output("ALTER TRIGGER %s ON %s.%s RENAME TO %s;" %
                           (cur_name, row[1], row[2], name,))
            else:
                output("-- Multiple occurrences of trigger: %s (for %s)" % (old_name, name,))
        else:
            output("-- Can't determine original trigger name: %s" % (name,))
    # Rules
    sample_c.execute("select pg_rewrite.oid, rulename, nspname, "
                     "relname, ev_attr, ev_type, is_instead "
                     "from pg_rewrite join pg_class on ev_class=pg_class.oid "
                     "join pg_namespace on relnamespace=pg_namespace.oid "
                     "where rulename like '%\\_\\_%'")
    ...
    for row in sample_c.fetchall():
        ...
        if match:
            ...
            current_c.execute("select pg_rewrite.oid, rulename "
                              "from pg_rewrite "
                              "join pg_class on ev_class=pg_class.oid "
                              "join pg_namespace on relnamespace=pg_namespace.oid "
                              "where (rulename=%s or rulename=%s) and nspname=%s "
                              "and relname=%s and ev_attr=%s and ev_type=%s "
                              "and is_instead=%s", (old_name,) + row[1:])
            ...
            if count == 0:
                ...
                if cur_name != name:
                    output("UPDATE pg_rewrite SET rulename='%s' WHERE oid=%s;" %
                           (name, rule_names[0][0],))
            else:
    ...
    # Indexes
    sample_c.execute("select ci.relname, nspname, c.relname from pg_index "
                     "join pg_class ci on indexrelid=ci.oid "
                     "join pg_class c on indrelid=c.oid "
                     "join pg_namespace on c.relnamespace=pg_namespace.oid "
                     "where ci.relname like 'ix_%'")
    for row in sample_c.fetchall():
        name = row[0]
        schema = row[1]
        table = row[2]
        ...
        if name.startswith(prefix):
            ...
            current_c.execute("select ci.relname from pg_index "
                              "join pg_class ci on indexrelid=ci.oid "
                              "join pg_class c on indrelid=c.oid "
                              "join pg_namespace on c.relnamespace=pg_namespace.oid "
                              "where (ci.relname=%s or ci.relname=%s or ci.relname=%s) "
                              "and nspname=%s and c.relname=%s", (old_name, old_name_2,) + row)
